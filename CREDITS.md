...
| --- | --- | --- | --- | --- |
| `quicknav` | `loongphy` | 提供暗色模式与回弹相关补丁 | 当前注册表注明补丁来源；主脚本由 `lueluelue2006` 持续演进 | `MIT（上游脚本声明）` |
...
| `chatgpt_download_file_fix` | `pengzhile` | ChatGPT 下载修复模块来源署名 | 当前注册表注明作者为 `pengzhile(linux.do)` | `未标注（内部脚本）` |
| `chatgpt_hide_feedback_buttons` | `zhong_little` | ChatGPT 隐藏点赞/点踩模块来源署名 | 当前注册表注明作者为 `zhong_little(linux.do)` | `未标注（内部脚本）` |
