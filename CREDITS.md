...
除下面明确列出的外部来源模块外，当前仓库里那些作者只写 `lueluelue2006` 的内部脚本，都按仓库作者本人编写处理，并统一按 `GPL-3.0-or-later` 发布。当前这份致谢名单里，`genspark_credit_balance` 是未标注模块中的唯一外部来源例外：它按外部来源脚本致谢给 `悟空(linux.do)`。
...
| `genspark_credit_balance` | `悟空(linux.do)` | Genspark 积分余量原始脚本作者；这是当前未标注模块里的唯一外部来源例外，其他同类内部脚本均归 `lueluelue2006` | 当前注册表注明其为原始脚本作者 | `许可证未声明` |
