<p align="center">
  <img src="assets/cover.png" width="320" alt="QuickNav" />
</p>

# QuickNav MV3

把现有的 QuickNav 油猴脚本封装成了一个 Chrome MV3 扩展
目前支持：ChatGPT / Gemini / Grok / Genspark / 文心一言国际版 / DeepSeek / Qwen / GLM 官网

## 安装

1. 打开 `chrome://extensions`
2. 开启「开发者模式」
3. 点击「加载已解压的扩展程序」
4. 选择本目录：`QuickNav-MV3`

## 使用

- 打开支持的网站页面后，右上角会出现 QuickNav 面板（与原脚本一致）
- `Option + 单击`：加图钉📌（图钉自动进收藏）
- 点击扩展图标，会打开一个简单信息面板：作者/版本 + 检查更新
  - 点击「配置」可进入设置页：控制在哪些网站启用 + 各网站默认 🔐（防自动滚动）（默认全锁）
  - 扩展「重新加载」后，已打开的页面通常需要刷新一次才会重新注入脚本

## 注意

- 如果你还启用了 Tampermonkey 同名脚本，建议先禁用，避免重复注入。

## 效果图
<img width="296" height="411" alt="iShot_2026-01-17_02 44 24" src="https://github.com/user-attachments/assets/379ffb33-b173-422d-8e4a-f1896ec7d3b8" />

## Logo
<img width="1024" height="1024" alt="封面" src="https://github.com/user-attachments/assets/6a407b24-11df-4187-bb3a-08f54266744c" />
