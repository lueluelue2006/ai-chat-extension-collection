(() => {
  ...
  const DUP_BURST_WINDOW_MS = 15000;
  ...
  const ring: DiagEvent[] = new Array(MAX_EVENTS);
  let head = 0;
  let size = 0;
  let droppedDuplicateMsgCount = 0;

  const duplicateMsgState = new Map<string, { windowStart: number; count: number }>();

  function clipText(value: any) {
  ...
  function normalizeTabId(value: any) {
    if (!Number.isFinite(value)) return null;
    const tabId = Math.trunc(value);
    return Number.isFinite(tabId) ? tabId : null;
  }

  function normalizeEvent(input: any): DiagEvent {
  ...
  function shouldDropDueToDuplicateMsg(event: DiagEvent) {
    const msg = event.msg;
    if (!msg) return false;
  ...
  function pushEvent(event: DiagEvent) {
    ring[head] = event;
