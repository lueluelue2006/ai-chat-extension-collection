(() => {
  ...
  const STATE_KEY = '__aichat_chatgpt_tab_queue_config_bridge_state_v1__';
  const SCOPE_API_KEY = '__aichat_quicknav_scope_v1__';
  const SCOPE_KEY = 'chatgpt_tab_queue_config_bridge';
  ...
  function flushPending() {
    if (disposed) return;
    try {
      const okA = writeBoolDataset(DS_QUEUE_ENABLED, pending.queueEnabled);
      const okB = writeBoolDataset(DS_CLEAR_ENABLED, pending.clearEnabled);
      const okC = writeBoolDataset(DS_QUICKNAV_MARK_ENABLED, pending.quicknavMarkEnabled);
      if (okA && okB && okC) {
        pending.tries = 0;
        return;
      }
  ...
  function applyFromSettings(settings) {
    ...
    try {
      ...
      const queueEnabled = typeof mods?.chatgpt_tab_queue_queue_shortcut === 'boolean' ? mods.chatgpt_tab_queue_queue_shortcut : true;
      const clearEnabled = typeof mods?.chatgpt_tab_queue_ctrl_c_clear === 'boolean' ? mods.chatgpt_tab_queue_ctrl_c_clear : true;
      const quicknavMarkEnabled =
        typeof mods?.chatgpt_tab_queue_quicknav_mark === 'boolean' ? mods.chatgpt_tab_queue_quicknav_mark : true;

      pending.queueEnabled = !!queueEnabled;
      pending.clearEnabled = !!clearEnabled && !!hasMetaKey;
      pending.quicknavMarkEnabled = !!quicknavMarkEnabled;
      flushPending();
    } catch {}
  }
  ...
  function ensureAttrGuard() {
    ...
    try {
      ...
      attrObserver = bridgeScope.observer(html, () => scheduleFlush(0), {
        attributes: true,
        attributeFilter: [
          'data-aichat-tab-queue-enabled',
          'data-aichat-tab-queue-ctrl-c-clear-enabled',
          'data-aichat-tab-queue-quicknav-mark-enabled'
        ]
      });
    } catch {}
  }
