(() => {
  ...
  const __aichatUsageMonitorStateKey = '__aichat_chatgpt_usage_monitor_v1__';
  let __aichatPrevUsageMonitorState = null;
  try {
    __aichatPrevUsageMonitorState = globalThis[__aichatUsageMonitorStateKey] || null;
  } catch {}
  try {
    if (
      __aichatPrevUsageMonitorState &&
      typeof __aichatPrevUsageMonitorState === 'object' &&
      typeof __aichatPrevUsageMonitorState.dispose === 'function'
    ) {
      __aichatPrevUsageMonitorState.dispose('reinject');
    } else if (__aichatPrevUsageMonitorState === true) {
      return;
    }
  } catch {}

  const __aichatUsageMonitorRuntimeId =
    `usage-monitor:${Date.now().toString(36)}:${Math.random().toString(36).slice(2, 10)}`;
  ...
  const __aichatScopeApiMainKey = '__aichat_quicknav_scope_main_v1__';

  function __aichatCreateFallbackScope() {
  ...
  function __aichatCreateManagedScope(scopeKey, singleton = false) {
    try {
      const scopeApi = globalThis[__aichatScopeApiMainKey];
      if (scopeApi && typeof scopeApi === 'object') {
        if (singleton && typeof scopeApi.createSingletonScope === 'function') {
          ...
          if (scope && typeof scope.dispose === 'function') return scope;
        }
    ...
    return __aichatCreateFallbackScope();
  }
  ...
  const __aichatScopedIntervalCancelById = new Map();
  const __aichatScopedTimeoutCancelById = new Map();

  function __aichatCreateTimerScope(kind) {
    const tag = `${kind || 'timer'}:${Date.now().toString(36)}:${Math.random().toString(36).slice(2, 8)}`;
    return __aichatCreateManagedScope(`chatgpt-usage-monitor-${tag}`);
  }

  function __aichatAttachTimerDisposer(owner, release) {
    if (owner === __aichatRouteScope) return __aichatAddRouteDisposer(release);
    return __aichatAddRuntimeDisposer(release);
  }

  function scopeInterval(scope, fn, ms, ...args) {
    const owner = scope && typeof scope.interval === 'function' ? scope : __aichatRuntimeScope;
    if (!owner || typeof fn !== 'function') return 0;

    const timerScope = __aichatCreateTimerScope('interval');
    if (!timerScope || typeof timerScope.interval !== 'function') return 0;

    let releaseTimerDisposer = () => void 0;
    let intervalId = 0;
    let released = false;
    const release = () => {
      if (released) return;
      released = true;
      __aichatScopedIntervalCancelById.delete(intervalId);
      try {
        releaseTimerDisposer();
      } catch {}
      try {
        timerScope.dispose();
      } catch {}
    ...
    intervalId = timerScope.interval(fn, ms, ...args);
    if (!intervalId) {
      release();
      return 0;
    }

    releaseTimerDisposer = __aichatAttachTimerDisposer(owner, release);
    __aichatScopedIntervalCancelById.set(intervalId, release);
    return intervalId;
  }

  function scopeTimeout(scope, fn, ms, ...args) {
    const owner = scope && typeof scope.timeout === 'function' ? scope : __aichatRuntimeScope;
    if (!owner || typeof fn !== 'function') return 0;

    const timerScope = __aichatCreateTimerScope('timeout');
    if (!timerScope || typeof timerScope.timeout !== 'function') return 0;
    ...
    const release = () => {
      if (released) return;
      released = true;
      __aichatScopedTimeoutCancelById.delete(timeoutId);
      try {
        releaseTimerDisposer();
      } catch {}
      try {
    ...
    releaseTimerDisposer = __aichatAttachTimerDisposer(owner, release);
    __aichatScopedTimeoutCancelById.set(timeoutId, release);
    return timeoutId;
  }

  function cancelScopedInterval(id) {
    if (!id) return;
    const release = __aichatScopedIntervalCancelById.get(id);
  ...
  function cancelScopedTimeout(id) {
    if (!id) return;
    const release = __aichatScopedTimeoutCancelById.get(id);
    if (typeof release === 'function') {
      release();
      return;
    }
  ...
  function scopeObserver(scope, arg0, arg1, arg2) {
    const owner = scope && typeof scope.observer === 'function' ? scope : __aichatRuntimeScope;
    if (!owner || typeof owner.observer !== 'function') return null;
    return owner.observer(arg0, arg1, arg2);
  }

  const __aichatRuntimeScope = __aichatCreateManagedScope('chatgpt-usage-monitor-runtime', true);
  let __aichatRouteScope = __aichatCreateManagedScope('chatgpt-usage-monitor-route');
  const __aichatRuntimeDisposers = new Set();
  const __aichatRouteDisposers = new Set();

  function __aichatRunDisposers(disposers) {
  ...
  function __aichatAddRuntimeDisposer(dispose) {
    if (typeof dispose !== 'function') return () => void 0;
    __aichatRuntimeDisposers.add(dispose);
    let active = true;
    return () => {
      if (!active) return;
      active = false;
      __aichatRuntimeDisposers.delete(dispose);
    };
  }

  function __aichatAddRouteDisposer(dispose) {
    if (typeof dispose !== 'function') return () => void 0;
    __aichatRouteDisposers.add(dispose);
    let active = true;
    return () => {
      if (!active) return;
      active = false;
      __aichatRouteDisposers.delete(dispose);
    };
  }

  function __aichatRunRuntimeDisposers() {
    __aichatRunDisposers(__aichatRuntimeDisposers);
  }

  function __aichatRunRouteDisposers() {
    __aichatRunDisposers(__aichatRouteDisposers);
  }

  function __aichatResetRouteScope() {
    __aichatRunRouteDisposers();
    try {
      if (__aichatRouteScope && typeof __aichatRouteScope.dispose === 'function') {
        __aichatRouteScope.dispose();
      }
    } catch {}
    __aichatRouteScope = __aichatCreateManagedScope('chatgpt-usage-monitor-route');
  }

  let __aichatDisposeMainRuntime = () => void 0;
  let __aichatUsageMonitorDisposed = false;

  function __aichatDisposeUsageMonitorRuntime(reason = 'dispose') {
    if (__aichatUsageMonitorDisposed) return;
    __aichatUsageMonitorDisposed = true;

    for (const release of Array.from(__aichatScopedIntervalCancelById.values())) {
      try {
        release();
      } catch {}
    }
    for (const release of Array.from(__aichatScopedTimeoutCancelById.values())) {
      try {
        release();
      } catch {}
    }
    ...
    try {
      __aichatDisposeMainRuntime(reason);
    } catch {}
    try {
      __aichatRunRouteDisposers();
    } catch {}
    ...
    try {
      __aichatRunRuntimeDisposers();
    } catch {}
    try {
      if (__aichatRuntimeScope && typeof __aichatRuntimeScope.dispose === 'function') {
        __aichatRuntimeScope.dispose();
      }
    } catch {}
  }

  try {
    Object.defineProperty(globalThis, __aichatUsageMonitorStateKey, {
      value: Object.freeze({
        version: 2,
        runtimeId: __aichatUsageMonitorRuntimeId,
        dispose: __aichatDisposeUsageMonitorRuntime
      }),
      configurable: true,
      enumerable: false,
      writable: false
    ...
    try {
      globalThis[__aichatUsageMonitorStateKey] = Object.freeze({
        version: 2,
        runtimeId: __aichatUsageMonitorRuntimeId,
        dispose: __aichatDisposeUsageMonitorRuntime
      });
  ...
  function __aichatQueueChromeSet(k, v) {
    ...
    __aichatGMChromeFlushTimer = scopeTimeout(__aichatRuntimeScope, () => {
  ...
  const __aichatMainMenuHandlers = Object.create(null);
  const __aichatMainMenuHandlerKeys = new Set();
  const __aichatMainMenuOwnerKey = String(__aichatUsageMonitorRuntimeId || 'usage-monitor').replace(/[^a-zA-Z0-9:_-]/g, '');

  function __aichatReleaseMainMenuHandlers() {
    for (const handlerKey of Array.from(__aichatMainMenuHandlerKeys)) {
      __aichatMainMenuHandlerKeys.delete(handlerKey);
      try {
        delete __aichatMainMenuHandlers[handlerKey];
      } catch {}
    }
  }
  ...
  function GM_registerMenuCommand(name, fn) {
    ...
    try {
      ...
      if (typeof reg === 'function') {
        return reg(n, fn, { group: __aichatMainMenuGroup, moduleId: __aichatMainMenuModuleId });
      }
    } catch {}

    const handlerKey = `chatgpt_usage_monitor:${__aichatMainMenuOwnerKey}:${n}`;
    __aichatMainMenuHandlers[handlerKey] = fn;
    __aichatMainMenuHandlerKeys.add(handlerKey);

    __aichatDispatchMainMenuRegister(n, handlerKey);
    // Retry in case the menu bridge isn't ready yet (both run at document_start).
    scopeTimeout(__aichatRuntimeScope, () => __aichatDispatchMainMenuRegister(n, handlerKey), 500);
    scopeTimeout(__aichatRuntimeScope, () => __aichatDispatchMainMenuRegister(n, handlerKey), 1500);
    return handlerKey;
  }
  ...
  function createRepeatedModelEntries(modelKeys, config) {
    ...
    (Array.isArray(modelKeys) ? modelKeys : []).forEach((modelKey) => {
  ...
  function isUsagePanelKnownModelKey(modelKey) {
    return MODEL_DISPLAY_ORDER.includes(canonicalizeUsageModelKey(modelKey));
  }
  ...
  function upgradeLegacyNominalUnlimitedData(data) {
    let changed = false;
    ...
    const groups = data && data.sharedQuotaGroups && typeof data.sharedQuotaGroups === "object" ? data.sharedQuotaGroups : null;
    if (groups) {
  ...
  function isAvailableUsageConfig(cfg, sharedGroups) {
    if (!cfg || typeof cfg !== "object") return false;
    if (cfg.sharedGroup) {
      const group = sharedGroups?.[cfg.sharedGroup];
      if (!group || typeof group !== "object") return false;
      return Number(group.quota) > 0;
    }
    return Number(cfg.quota) > 0;
  }
  ...
  var PLAN_CONFIGS = {
    free: {
      name: "Free",
      sharedQuotaGroups: {
        "free-thinking-shared": {
          quota: 1,
          windowType: "hour5",
          displayName: "Free思考共用池"
        },
        "free-instant-shared": {
          quota: 10,
          windowType: "hour5",
      ...
      models: {
        ...createRepeatedModelEntries(PRO_FAMILY_MODEL_KEYS, { quota: 0, windowType: "monthly" }),
    ...
    k12_teacher: {
      ...
      sharedQuotaGroups: {
        ...
        "k12-instant-shared": {
          quota: LEGACY_NOMINAL_UNLIMITED_QUOTA,
          windowType: LEGACY_NOMINAL_UNLIMITED_WINDOW_TYPE,
    ...
    team: {
      ...
      models: {
        ...createRepeatedModelEntries(PRO_FAMILY_MODEL_KEYS, { sharedGroup: "team-premium-shared" }),
        "gpt-4-5": { quota: 0, windowType: "daily" },
    ...
    edu: {
      ...
      models: {
        ...createRepeatedModelEntries(PRO_FAMILY_MODEL_KEYS, { sharedGroup: "edu-premium-shared" }),
        "gpt-4-5": { quota: 0, windowType: "daily" },
    ...
    enterprise: {
      ...
      models: {
        ...createRepeatedModelEntries(PRO_FAMILY_MODEL_KEYS, { sharedGroup: "enterprise-premium-shared" }),
        "gpt-4-5": { quota: 0, windowType: "daily" },
    ...
    pro: {
      ...
      models: {
        ...createRepeatedModelEntries(PRO_FAMILY_MODEL_KEYS, { sharedGroup: "pro-premium-shared" }),
  ...
  var Storage = {
    ...
    get() {
      ...
      upgradeLegacyNominalUnlimitedData(data);
      try {
        mergeAliasedUsageModels(data.models);
      } catch {
      }
      ...
      const currentPlanConfig = PLAN_CONFIGS[data.planType] || PLAN_CONFIGS.team;
      const newModels = Object.keys(currentPlanConfig.models || {}).concat("alpha");
      newModels.forEach((modelId) => {
        if (data.models[modelId]) return;
        const cfg = currentPlanConfig.models?.[modelId];
        if (cfg && typeof cfg === "object") {
          data.models[modelId] = { requests: [] };
          if (cfg.sharedGroup) {
            data.models[modelId].sharedGroup = cfg.sharedGroup;
          } else {
            data.models[modelId].quota = cfg.quota;
            data.models[modelId].windowType = cfg.windowType;
          }
        } else if (modelId === "alpha") {
      ...
      if (data.sharedQuotaGroups && typeof data.sharedQuotaGroups === "object") {
        Object.values(data.sharedQuotaGroups).forEach((group) => {
          ...
          upgradeLegacyNominalUnlimitedEntry(group);
        });
      }
  ...
  let __aichatToastHideTimerId = 0;
  let __aichatToastRemoveTimerId = 0;
  function clearToastTimers() {
    if (__aichatToastShowTimerId) {
      cancelScopedTimeout(__aichatToastShowTimerId);
      __aichatToastShowTimerId = 0;
    }
    if (__aichatToastHideTimerId) {
      cancelScopedTimeout(__aichatToastHideTimerId);
      __aichatToastHideTimerId = 0;
    }
    if (__aichatToastRemoveTimerId) {
      cancelScopedTimeout(__aichatToastRemoveTimerId);
      __aichatToastRemoveTimerId = 0;
    }
  }
  function showToast(message, type = "success") {
    ...
    __aichatToastShowTimerId = scopeTimeout(__aichatRouteScope, () => {
      __aichatToastShowTimerId = 0;
      toast.classList.add("show");
    }, 10);
    __aichatToastHideTimerId = scopeTimeout(__aichatRouteScope, () => {
      __aichatToastHideTimerId = 0;
      toast.classList.remove("show");
      __aichatToastRemoveTimerId = scopeTimeout(__aichatRouteScope, () => {
        __aichatToastRemoveTimerId = 0;
        toast.remove();
      }, 300);
    }, 3e3);
  ...
  function exportUsageData() {
    ...
    a.click();
    let cleanedUp = false;
    let releaseCleanupDisposer = () => void 0;
    const cleanupExport = () => {
      if (cleanedUp) return;
      cleanedUp = true;
      try {
        releaseCleanupDisposer();
      } catch {}
      try {
    ...
    releaseCleanupDisposer = __aichatAddRouteDisposer(() => {
      cleanupExport();
    });
  ...
  `);
				  ...
				  function __aichatIsUsageConversationRequest(url, method) {
				    return String(method || "").toUpperCase() === "POST" && /\/backend-api\/(?:f\/)?conversation(?:\?|$)/.test(String(url || ""));
				  }
				  function __aichatIsSteerTurnUsageRequest(url, method) {
				    return String(method || "").toUpperCase() === "POST" && /\/backend-api\/f\/steer_turn(?:\?|$)/.test(String(url || ""));
				  }
				  function __aichatResolveUsageModelKey(ctx) {
				    // Prefer the actual outgoing body: fetch hub payload snapshots can lag during
				    // steer_turn follow-ups and otherwise mis-attribute the request to a stale model.
				    const requestScopedKey = getUsageModelKeyFromCookieOrBody(ctx?.init?.body);
				    if (requestScopedKey) return requestScopedKey;
				    try {
				      const payload = ctx?.conversation?.payload;
				      if (payload && typeof payload === "object" && typeof payload.model === "string" && payload.model) {
				        return normalizeUsageModelKey(payload.model);
				      }
				    } catch {}
				    return getUsageModelKeyFromCookieOrBody(null);
				  }
				  function __aichatScheduleUsageRecord(modelKey) {
				  ...
				  function __aichatRecordUsageFromRequestContext(ctx) {
				    try {
				      if (__aichatShouldSkipUsageCounting(ctx)) return false;
				      const modelKey = __aichatResolveUsageModelKey(ctx);
				      return __aichatScheduleUsageRecord(modelKey);
				    } catch {}
				    return false;
				  }
			      ...
			      consumerBase && typeof consumerBase.registerConsumer === "function"
			        ...
			        : hub && typeof hub.register === "function"
			          ...
			          : null;
			    const hasFetchHub = !!registerConsumer;
			    if (registerConsumer) {
			      if (!__aichatFetchHubUnsub) {
			        ...
			        const unsub = registerConsumer('chatgpt-usage-monitor', {
			          ...
			          priority: 200,
				          onConversationStart: (ctx) => {
				            __aichatRecordUsageFromRequestContext(ctx);
			          },
		        });
		        if (typeof unsub === 'function') {
		          __aichatFetchHubUnsub = unsub;
		          __aichatAddRuntimeDisposer(() => {
		            if (!__aichatFetchHubUnsub) return;
		            try {
		              __aichatFetchHubUnsub();
		            } catch {}
		            __aichatFetchHubUnsub = null;
		          });
		        }
			      }
		    }
    ...
    try {
		      const prev = targetWindow[__aichatUsageMonitorFetchPatchStateKey] || null;
		      if (prev && typeof prev.restore === 'function') {
		        prev.restore('reinstall');
		      }
    } catch {}
			    ...
			    const wrapped = new Proxy(originalFetch, {
			      apply: async function(target, thisArg, args) {
			        ...
			        try {
			          ...
			          const shouldTrackSteerTurn = __aichatIsSteerTurnUsageRequest(fetchUrl, requestMethod);
			          const shouldTrackConversation = !hasFetchHub && __aichatIsUsageConversationRequest(fetchUrl, requestMethod);
			          if (shouldTrackConversation || shouldTrackSteerTurn) {
    ...
    const patchState = {
      ownerId: __aichatUsageMonitorRuntimeId,
      restore: () => {
        try {
          if (targetWindow.fetch === wrapped) {
    ...
    try {
      Object.defineProperty(targetWindow, __aichatUsageMonitorFetchPatchStateKey, {
        value: patchState,
        configurable: true,
        enumerable: false,
        writable: true
      ...
      try {
        targetWindow[__aichatUsageMonitorFetchPatchStateKey] = patchState;
      } catch {}
    }
    __aichatAddRuntimeDisposer(() => {
      try {
      ...
      try {
        const active = targetWindow[__aichatUsageMonitorFetchPatchStateKey] || null;
        if (active === patchState) {
          targetWindow[__aichatUsageMonitorFetchPatchStateKey] = null;
        }
      } catch {}
    });
  }
  ...
  function collectSharedGroupUsage(groupId, now = Date.now()) {
    ...
    Object.entries(usageData.models || {}).forEach(([key, model]) => {
      ...
      model.requests
        ...
        .forEach((ts) => {
          activeRequests.push({ ts, modelKey: key });
        });
    });
    ...
    return {
	  ...
	  // src/tracking/modelRouting.js
	    ...
	    if (originalModelId === "auto") {
	      return "gpt-5-3-instant";
	    }
	    try {
	      const plan = usageData && usageData.planType || "team";
	      if (originalModelId === "gpt-4-5" && plan !== "pro") return "gpt-5-3-instant";
	    } catch {
	    }
      ...
      var TextScrambler = (() => {
        ...
        var r = class {
          constructor(t, e = {}) {
            ...
            this.mask = this.originalText.split(" ").map((i) => "\u00A0".repeat(i.length)).join(" ");
          ...
          _getEased(t) {
            ...
            e = Math.pow(e, 2);
            return this.config.reverse ? 1 - e : e;
          }
            ...
            if (Math.random() < 0.5 && t > 0 && t < 1) {
              for (let i = 0; i < 20; i++) {
                ...
                if (!(a < 0 || a >= this.totalChars) && this.currentMask[a] !== " ") {
                  const h = Math.random() > this.config.randomThreshold ? this.originalText[a] : M(this.config.reverse, this.config.scrambleSymbols);
          ...
          _composeOutput(t, e, s) {
            ...
            if (this.config.reverse) {
              ...
              i = this.originalText.slice(0, e) + this.currentMask.slice(e, e + s) + this.mask.slice(e + s);
            }
            return i;
          }
          _updateTextNodes(t) {
            ...
            for (let s = 0; s < this.textNodes.length; s++) {
              const i = this.nodeLengths[s];
              this.textNodes[s].nodeValue = t.slice(e, e + i);
              e += i;
            }
          }
          _tick = (t) => {
            ...
            const e = t - this._startTime;
            const s = Math.min(e / this.config.duration, 1);
            const i = this._getEased(s);
            const o = Math.floor(this.totalChars * i);
            const a = Math.floor(2 * (0.5 - Math.abs(s - 0.5)) * this.scrambleRange);
            this._updateScramble(s, o, a);
            const h = this._composeOutput(s, o, a);
            this._updateTextNodes(h);
            if (s < 1) {
          ...
          start() {
            ...
            if (this.config.delay) {
              this._delayTimerId = scopeTimeout(__aichatRouteScope, () => {
                this._delayTimerId = null;
                if (!this._running) return;
                this._animationFrame = requestAnimationFrame(this._tick);
              }, this.config.delay);
          ...
          stop() {
            if (this._delayTimerId) {
              cancelScopedTimeout(this._delayTimerId);
              this._delayTimerId = null;
            }
  ...
  function generateWeeklyReport() {
    ...
    const sortedModelEntries = MODEL_DISPLAY_ORDER.filter((modelKey) => usageData.models[modelKey]).map((modelKey) => [modelKey, usageData.models[modelKey]]);
  ...
  function mergeUnknownModelsForHtml(report) {
    try {
      const KNOWN = /* @__PURE__ */ new Set([
        ...
        ...MODEL_DISPLAY_ORDER,
        // 再补充兼容显示顺序外但“已知”的模型键
        ...Object.keys(MODEL_KEY_ALIASES),
        "alpha"
      ]);
      const targetKey = "gpt-5-3-instant";
...
<\/script>
  ...
  // src/ui/monitor.js
  const __aichatTextAnimatorByElement = new WeakMap();
  const __aichatActiveTextAnimators = new Set();
  function __aichatStopTextAnimator(animator) {
    if (!animator || typeof animator.stop !== "function") return;
    try {
      animator.stop();
    } catch {}
    __aichatActiveTextAnimators.delete(animator);
  }
  function __aichatStopTextAnimatorForElement(el) {
    if (!el) return;
    const existingAnimator = __aichatTextAnimatorByElement.get(el);
    if (!existingAnimator) return;
    __aichatTextAnimatorByElement.delete(el);
    __aichatStopTextAnimator(existingAnimator);
  }
  function __aichatStopAllTextAnimators() {
    for (const animator of Array.from(__aichatActiveTextAnimators)) {
      __aichatStopTextAnimator(animator);
    }
    __aichatActiveTextAnimators.clear();
  }
  function animateText(el, config) {
    if (!el) return;
    __aichatStopTextAnimatorForElement(el);
    try {
      ...
      if (typeof animator.stop === "function") {
        __aichatTextAnimatorByElement.set(el, animator);
        __aichatActiveTextAnimators.add(animator);
      }
    } catch (e) {
  ...
  `;
    ...
    const modelCounts = canonicalModelEntries.map(([key, model]) => {
      ...
      if (model.sharedGroup) {
        ...
        isAvailable = Number(model.quota) > 0;
      }
...
这将更新所有模型的配额和时间窗口设置。`)) {
  ...
  function __aichatUiPosApi() {
    ...
    function enableRightTopDrag(element, handle, opts = {}) {
      ...
      const ownerScope = opts && typeof opts === "object" && opts.scope && typeof opts.scope.on === "function" ? opts.scope : null;
      const bind = (target, type, listener, eventOpts) => {
        if (ownerScope) return scopeOn(ownerScope, target, type, listener, eventOpts);
  ...
  const __aichatDragCleanupByElement = new WeakMap();
  function setupDraggable(element) {
    const api = __aichatUiPosApi();
    if (!api || typeof api.enableRightTopDrag !== "function") return;

    try {
      const prevDispose = __aichatDragCleanupByElement.get(element);
    ...
    if (handle) {
      const off = api.enableRightTopDrag(element, handle, {
        thresholdPx: 6,
        scope: __aichatRouteScope,
        isInteractive,
        onDragEnd: (e) => {
          commitPosition();
          try {
      ...
      if (typeof off === "function") dragDisposers.push(off);
    }

    const off = api.enableRightTopDrag(element, element, {
      thresholdPx: 6,
      scope: __aichatRouteScope,
      shouldStart: (e) => {
        try {
    ...
    const disposeDraggable = () => {
      while (dragDisposers.length) {
        const dispose = dragDisposers.pop();
    ...
    try {
      __aichatDragCleanupByElement.set(element, disposeDraggable);
    } catch {}
    __aichatAddRouteDisposer(() => {
      disposeDraggable();
      try {
        __aichatDragCleanupByElement.delete(element);
      } catch {}
    });
  }
  var _keyboardShortcutsOff = null;
  function setupKeyboardShortcuts() {
    ...
    _keyboardShortcutsOff = scopeOn(__aichatRuntimeScope, document, "keydown", handleShortcut, true);
  }
  function disposeKeyboardShortcuts() {
    if (!_keyboardShortcutsOff) return;
    try {
      _keyboardShortcutsOff();
    } catch {}
    _keyboardShortcutsOff = null;
  }
  __aichatAddRuntimeDisposer(disposeKeyboardShortcuts);
  ...
  function cancelPendingMonitorMinimized() {
    ...
    if (_minimizeTimerId) {
      try {
        cancelScopedTimeout(_minimizeTimerId);
      } catch {
      }
  ...
  function requestMonitorMinimized(nextMinimized) {
    ...
    _minimizeTimerId = scopeTimeout(__aichatRouteScope, () => {
      _minimizeTimerId = null;
    ...
    if (_uiUpdateIntervalId) {
      cancelScopedInterval(_uiUpdateIntervalId);
      _uiUpdateIntervalId = null;
    }
    _uiUpdateIntervalId = scopeInterval(__aichatRouteScope, updateUI, 6e4);
  }
    ...
    const offDataChanged = onDataChanged(() => {
    ...
    __aichatAddRuntimeDisposer(offDataChanged);

    let _pendingInitTimerId = null;
    let _pendingInitDueAt = 0;
    let _pendingBodyWaitTimerId = null;
    let _initializedOnce = false;

    function clearPendingInitializeTimers() {
      if (_pendingInitTimerId) {
        try {
          cancelScopedTimeout(_pendingInitTimerId);
        } catch {}
        _pendingInitTimerId = null;
      }
      _pendingInitDueAt = 0;
      if (_pendingBodyWaitTimerId) {
        try {
          cancelScopedTimeout(_pendingBodyWaitTimerId);
        } catch {}
        _pendingBodyWaitTimerId = null;
      }
    }

    function disposeRouteRuntime(_reason = "route-change", { recreateScope = true } = {}) {
      cancelPendingMonitorMinimized();
      clearPendingInitializeTimers();
      __aichatStopAllTextAnimators();
      if (_uiUpdateIntervalId) {
        try {
          cancelScopedInterval(_uiUpdateIntervalId);
        } catch {}
      ...
      if (recreateScope) {
        __aichatResetRouteScope();
      } else {
        __aichatRunRouteDisposers();
      }
    }

    let __aichatRouteWatcherUnsub = null;
    const clearRouteWatcherUnsub = () => {
      if (!__aichatRouteWatcherUnsub) return;
      try {
        __aichatRouteWatcherUnsub();
      } catch {}
      __aichatRouteWatcherUnsub = null;
    };
    __aichatDisposeMainRuntime = () => {
      disposeRouteRuntime("dispose", { recreateScope: false });
      clearRouteWatcherUnsub();
    };
    ...
    function scheduleInitialize(delay = 300) {
      ...
      _pendingInitTimerId = scopeTimeout(__aichatRouteScope, () => {
        _pendingInitTimerId = null;
        _pendingInitDueAt = 0;
        initialize();
    ...
    function initialize() {
      if (!document?.body) {
        if (_pendingBodyWaitTimerId) return;
        _pendingBodyWaitTimerId = scopeTimeout(__aichatRouteScope, () => {
          _pendingBodyWaitTimerId = null;
          initialize();
        }, 300);
        return;
      ...
      scopeTimeout(__aichatRouteScope, () => {
        const monitor = document.getElementById("chatUsageMonitor");
        if (monitor) {
          monitor.style.setProperty("left", STYLE.spacing.lg, "important");
          monitor.style.setProperty("bottom", "100px", "important");
	    ...
	    if (document.readyState === "loading") {
	      scopeOn(__aichatRuntimeScope, window, "DOMContentLoaded", () => scheduleInitialize(0), { once: true });
	    } else {
	      scheduleInitialize(0);
	    }

    const bindRouteWatcherUnsub = (unsub) => {
      if (typeof unsub !== "function") return false;
      __aichatRouteWatcherUnsub = unsub;
      __aichatAddRuntimeDisposer(() => {
        clearRouteWatcherUnsub();
      });
      return true;
    ...
    function installRouteWatcher() {
      const onRoute = () => {
        ...
        disposeRouteRuntime("route-change", { recreateScope: true });
      ...
      try {
        ...
        if (core && typeof core.onRouteChange === "function") {
          if (bindRouteWatcherUnsub(core.onRouteChange(onRoute))) return;
        }
      } catch {}
      ...
      try {
        ...
        if (bridge && typeof bridge.ensureRouteListener === "function" && typeof bridge.on === "function") {
          ...
          if (bindRouteWatcherUnsub(bridge.on("routeChange", onRoute))) return;
        }
      } catch {}

      scopeOn(__aichatRuntimeScope, window, "popstate", onRoute);
      scopeOn(__aichatRuntimeScope, window, "hashchange", onRoute);
      ...
      scopeInterval(__aichatRuntimeScope, () => {
        try {
    ...
    function installEnsureMonitor() {
      ...
      const scheduleEnsure = (ms) => {
        if (__aichatEnsureTimer) return;
        __aichatEnsureTimer = scopeTimeout(__aichatRuntimeScope, () => {
          __aichatEnsureTimer = 0;
          try {
            if (document.hidden) return scheduleEnsure(15000);
            if (isSilent()) return scheduleEnsure(15000);
            if (!document.getElementById("chatUsageMonitor")) scheduleInitialize(0);
          } catch {}
      ...
      const bootstrapEnsure = () => {
        try {
          if (installEnsureMo()) return;
          __aichatEnsureTries += 1;
          if (__aichatEnsureTries < 20) {
            __aichatEnsureBootstrapTimer = scopeTimeout(__aichatRuntimeScope, () => {
              __aichatEnsureBootstrapTimer = 0;
              bootstrapEnsure();
            }, 500);
          }
        } catch {}
