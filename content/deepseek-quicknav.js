(function () {
  ...
  const DEEPSEEK_KEYS_BOUND_KEY = '__deepseekQuicknavKeysBound';
  const DEEPSEEK_KEYS_BOUND_LEGACY_KEY = '__cgptKeysBound';
  const DEEPSEEK_SCROLL_LOCK_USER_INTENTS_BOUND_KEY = '__deepseekQuicknavScrollLockUserIntentsBound';
  const DEEPSEEK_SCROLL_LOCK_USER_INTENTS_BOUND_LEGACY_KEY = '__cgptScrollLockUserIntentsBound';
  const DEEPSEEK_SCROLL_GUARDS_INSTALLED_KEY = '__deepseekQuicknavScrollGuardsInstalled';
  const DEEPSEEK_SCROLL_GUARDS_INSTALLED_LEGACY_KEY = '__cgptScrollGuardsInstalled';
  const DEEPSEEK_SCROLL_LOCK_BOUND_KEY = '__deepseekQuicknavScrollLockBound';
  const DEEPSEEK_SCROLL_LOCK_BOUND_LEGACY_KEY = '__cgptScrollLockBound';
  const DEEPSEEK_PIN_BOUND_KEY = '__deepseekQuicknavPinBound';
  const DEEPSEEK_PIN_BOUND_LEGACY_KEY = '__cgptPinBound';
  const DEEPSEEK_SEND_EVENTS_BOUND_KEY = '__deepseekQuicknavSendEventsBoundV1';

  function isDeepseekKeysBound() {
    return isCompatBoolFlagSet(DEEPSEEK_KEYS_BOUND_KEY, DEEPSEEK_KEYS_BOUND_LEGACY_KEY);
  }

  function setDeepseekKeysBound(value) {
    return setCompatBoolFlag(DEEPSEEK_KEYS_BOUND_KEY, DEEPSEEK_KEYS_BOUND_LEGACY_KEY, value);
  }

  function isDeepseekScrollLockUserIntentsBound() {
    return isCompatBoolFlagSet(DEEPSEEK_SCROLL_LOCK_USER_INTENTS_BOUND_KEY, DEEPSEEK_SCROLL_LOCK_USER_INTENTS_BOUND_LEGACY_KEY);
  }

  function setDeepseekScrollLockUserIntentsBound(value) {
    return setCompatBoolFlag(
      DEEPSEEK_SCROLL_LOCK_USER_INTENTS_BOUND_KEY,
      DEEPSEEK_SCROLL_LOCK_USER_INTENTS_BOUND_LEGACY_KEY,
      value
    );
  }

  function isDeepseekScrollGuardsInstalled() {
    return isCompatBoolFlagSet(DEEPSEEK_SCROLL_GUARDS_INSTALLED_KEY, DEEPSEEK_SCROLL_GUARDS_INSTALLED_LEGACY_KEY);
  }

  function setDeepseekScrollGuardsInstalled(value) {
    return setCompatBoolFlag(DEEPSEEK_SCROLL_GUARDS_INSTALLED_KEY, DEEPSEEK_SCROLL_GUARDS_INSTALLED_LEGACY_KEY, value);
  }

  function isDeepseekScrollLockBound() {
    return isCompatBoolFlagSet(DEEPSEEK_SCROLL_LOCK_BOUND_KEY, DEEPSEEK_SCROLL_LOCK_BOUND_LEGACY_KEY);
  }

  function setDeepseekScrollLockBound(value) {
    return setCompatBoolFlag(DEEPSEEK_SCROLL_LOCK_BOUND_KEY, DEEPSEEK_SCROLL_LOCK_BOUND_LEGACY_KEY, value);
  }

  function isDeepseekPinBound() {
    return isCompatBoolFlagSet(DEEPSEEK_PIN_BOUND_KEY, DEEPSEEK_PIN_BOUND_LEGACY_KEY);
  }

  function setDeepseekPinBound(value) {
    return setCompatBoolFlag(DEEPSEEK_PIN_BOUND_KEY, DEEPSEEK_PIN_BOUND_LEGACY_KEY, value);
  }

  function isDeepseekSendEventsBound() {
    try {
      return !!window[DEEPSEEK_SEND_EVENTS_BOUND_KEY];
    } catch {
      return false;
    }
  }

  function setDeepseekSendEventsBound(value) {
    const normalized = !!value;
    try { window[DEEPSEEK_SEND_EVENTS_BOUND_KEY] = normalized; } catch {}
    return normalized;
  }

  // 全局调试函数，用户可在控制台调用
  const deepseekQuicknavDebugApi = {
    forceRefresh: () => {
      console.log('DeepSeek QuickNav: 手动强制刷新');
      TURN_SELECTOR = null;
    ...
    checkOverlap: () => {
      ...
      console.log(`键盘事件已绑定: ${isDeepseekKeysBound()}`);
      console.log(`正在启动中: ${deepseekQuicknavBooting}`);
      if (panels.length > 1) {
      ...
      return {
        ...
        keysBound: isDeepseekKeysBound(),
        booting: deepseekQuicknavBooting
      };
    },
    testObserver: () => {
  ...
  window.deepseekQuicknavDebug = deepseekQuicknavDebugApi;
  window.deepseekNavDebug = deepseekQuicknavDebugApi;
  window.chatGptNavDebug = deepseekQuicknavDebugApi; // backward compatible alias
  ...
  let deepseekQuicknavBooting = false;
  ...
  function init() {
    ...
    const boot = () => {
      ...
      if (document.getElementById('cgpt-compact-nav')) {
        if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 面板已存在，跳过创建');
        return;
      }
      if (deepseekQuicknavBooting) {
        if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 正在启动中，跳过重复创建');
        return;
      }

      deepseekQuicknavBooting = true;
      try {
        if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 开始创建面板');
        ...
        scheduleRefresh(ui);
        if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 面板创建完成');
      } finally {
        deepseekQuicknavBooting = false;
      }
    };
  ...
  function detectUrlChange(payload = null) {
    ...
    if (nextHref !== currentUrl) {
      if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: URL变化，清理旧实例', currentUrl, '->', nextHref);
      currentUrl = nextHref;
      ...
      if (oldNav) {
        if (oldNav._ui) {
          ...
          if (oldNav._ui._forceRefreshTimer) {
            ...
            if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 已清理定时器');
          }
          // 断开MutationObserver
          if (oldNav._ui._mo) {
            try {
              oldNav._ui._mo.disconnect();
              if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 已断开MutationObserver');
            } catch (e) {
              if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 断开MutationObserver失败', e);
            }
          }
        ...
        oldNav.remove();
        if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 已移除旧面板');
      }
      // 重置"正在启动"标志，避免新页面被卡住
      deepseekQuicknavBooting = false;
      lastTurnCount = 0;
  ...
  (function installRouteWatcher() {
    try {
      if (QUICKNAV_ROUTE_WATCH && typeof QUICKNAV_ROUTE_WATCH.installBridgeFirstWatcher === 'function') {
        const installed = QUICKNAV_ROUTE_WATCH.installBridgeFirstWatcher({
          target: window,
          unsubKey: '__quicknavDeepseekRouteUnsubV1',
          pollKey: '__quicknavDeepseekRoutePollV1',
          pollMs: 1200,
    ...
    try {
      if (window.__quicknavDeepseekRouteUnsubV1 && typeof window.__quicknavDeepseekRouteUnsubV1 === 'function') return;
      ...
      if (bridge && typeof bridge.ensureRouteListener === 'function' && typeof bridge.on === 'function') {
        ...
        window.__quicknavDeepseekRouteUnsubV1 = bridge.on('routeChange', (eventPayload) => detectUrlChange(eventPayload || null));
        return;
      }
    } catch {}
    ...
    try {
      if (window.__quicknavDeepseekRoutePollV1) return;
      window.__quicknavDeepseekRoutePollV1 = window.setInterval(() => detectUrlChange({ href: location.href, reason: 'poll' }), 1200);
    } catch {}
  })();
  ...
  function qsTurns(root = document) {
    ...
    if (DEBUG || window.DEBUG_TEMP) {
      console.log('DeepSeek QuickNav Debug: 检测对话选择器');
    ...
    for (const selector of selectors) {
      ...
      if (els.length) {
        ...
        if (DEBUG || window.DEBUG_TEMP) console.log(`DeepSeek QuickNav: 使用选择器 ${selector}, 找到 ${els.length} 个对话`);
        return Array.from(els);
      }
    }

    if (DEBUG || window.DEBUG_TEMP) {
      console.log('DeepSeek QuickNav Debug: 所有预设选择器都失效，尝试智能检测');
    ...
    for (const fallbackSelector of fallbackSelectors) {
      ...
      if (candidates.length > 0) {
        if (DEBUG || window.DEBUG_TEMP) console.log(`DeepSeek QuickNav: Fallback选择器 ${fallbackSelector} 找到 ${candidates.length} 个候选对话`);
        return candidates;
      }
    }

    if (DEBUG) console.log('DeepSeek QuickNav: 所有检测方法均失效');
    return [];
  }
  ...
  function buildIndex(turnsOverride) {
    ...
    turnIdToPos.clear();
    if (!turns.length) {
      if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 没有找到任何对话元素');
      return [];
    }

    if (DEBUG) console.log(`DeepSeek QuickNav: 开始分析 ${turns.length} 个对话元素`);
    ...
    for (let i = 0; i < turns.length; i++) {
      ...
      if (DEBUG && i < 3) {
        console.log(`DeepSeek QuickNav Debug - 元素 ${i}:`, {
          element: el,
          testId: attrTestId,
          isUser,
          isAssistant,
      ...
      if (!isUser && !isAssistant) {
        if (DEBUG && i < 5) console.log(`DeepSeek QuickNav: 元素 ${i} 角色识别失败`);
        continue;
      }
      ...
      if (!preview) {
        if (DEBUG && i < 5) console.log(`DeepSeek QuickNav: 元素 ${i} 无法提取预览文本`);
        continue;
      }
    ...
    if (DEBUG) console.log(`DeepSeek QuickNav: 成功识别 ${list.length} 个对话 (用户: ${u}, 助手: ${a})`);
    return list;
  }
...
`;
      ...
      if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 已创建样式');
    } else {
      if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 样式已存在，跳过创建');
    }

    // 启动前清理多余面板（保险丝）
    const existingPanels = document.querySelectorAll('#cgpt-compact-nav');
    if (existingPanels.length > 0) {
      if (DEBUG || window.DEBUG_TEMP) console.log(`DeepSeek QuickNav: 发现 ${existingPanels.length} 个已存在的面板，清理中...`);
      existingPanels.forEach((panel, index) => {
        if (index > 0) { // 保留第一个，删除其他
          panel.remove();
          if (DEBUG || window.DEBUG_TEMP) console.log(`DeepSeek QuickNav: 已删除重复面板 ${index}`);
        }
      });
      // 如果已经有面板存在，直接返回现有的
      if (existingPanels.length > 0) {
        ...
        if (existingNav._ui) {
          if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 返回已存在的面板');
    ...
    try {
      ...
      if (DEBUG || window.DEBUG_TEMP) console.error('DeepSeek QuickNav: 布局管理器初始化失败', err);
    }
    enableDrag(nav, {
  ...
  function createLayoutManager(nav) {
    ...
    function scheduleEvaluation(reason) {
      ...
      state.rafId = requestAnimationFrame(() => {
        state.rafId = 0;
        state.pendingEval = false;
        try { evaluateNow(reason); } catch (err) { if (DEBUG || window.DEBUG_TEMP) console.error('DeepSeek QuickNav layout evaluate error:', err); }
      });
    }
  ...
  function refreshIndex(ui, { soft = false, force = false } = {}) {
    ...
    if (DEBUG) console.log('DeepSeek QuickNav: turns', next.length, '(含📌)');
  ...
  function wirePanel(ui) {
    ...
    if (refreshBtn) {
      refreshBtn.addEventListener('click', (e) => {
        if (e.shiftKey) {
          // Shift+点击 = 强制重新扫描
          if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 强制重新扫描 (清除缓存选择器)');
      ...
      refreshBtn.addEventListener('contextmenu', (e) => {
        e.preventDefault();
        if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 右键强制重新扫描');
        TURN_SELECTOR = null;
        const originalBg = refreshBtn.style.background;
        const originalColor = refreshBtn.style.color;
        refreshBtn.style.background = 'var(--cgpt-nav-accent-subtle)';
    ...
    if (!isDeepseekKeysBound()) {
      const onKeydown = (e) => {
        ...
        if (e.altKey && e.key === '/') {
          const nav = document.getElementById('cgpt-compact-nav');
          const activeUi = nav?._ui || ui;
          const list = activeUi?.nav?.querySelector('.compact-list');
          const toggleText = activeUi?.nav?.querySelector('.compact-toggle .toggle-text');
          if (!list) return;
          const isHidden = list.getAttribute('data-hidden') === '1';
          if (isHidden) {
      ...
      setDeepseekKeysBound(true);
      if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 已绑定键盘事件');
    } else {
      if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 键盘事件已存在，跳过绑定');
    }
  }
  ...
  function getTurnsContainer() {
    ...
    if (!nodes.length) {
      ...
      if (DEBUG && potentialContainers.length > 1) {
        console.log('DeepSeek QuickNav: 没有找到对话，使用备用容器:', potentialContainers[0]);
      }
    ...
    while (a) {
      if (nodes.every(n => a.contains(n))) {
        if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 对话容器:', a);
        return a;
      }
      a = a.parentElement;
    }
  ...
  function bindScrollLockUserIntents() {
    if (isDeepseekScrollLockUserIntentsBound()) return;
    setDeepseekScrollLockUserIntentsBound(true);
  ...
  function installScrollGuards() {
    if (isDeepseekScrollGuardsInstalled()) return;
    setDeepseekScrollGuardsInstalled(true);
  ...
  function watchSendEvents(ui) {
    if (isDeepseekSendEventsBound()) return;
    setDeepseekSendEventsBound(true);
    ...
    const triggerSendRefresh = (reason) => {
      const targetUi = getUi();
      if (!targetUi) return;
      if (DEBUG || window.DEBUG_TEMP) console.log(`DeepSeek QuickNav: ${reason}，启动突发刷新`);
      armScrollLockGuard(2200);
      startBurstRefresh(targetUi);
    };

    // 点击发送按钮
    document.addEventListener('click', (e) => {
      if (e.target && e.target.closest && e.target.closest('[data-testid="send-button"]')) {
        triggerSendRefresh('检测到发送按钮点击');
      }
    }, true);
    ...
    document.addEventListener('visibilitychange', () => {
      if (!document.hidden) {
        const targetUi = getUi();
        if (!targetUi) return;
        if (DEBUG || window.DEBUG_TEMP) console.log('DeepSeek QuickNav: 页面重新可见，强制刷新');
        scheduleRefresh(targetUi, { force: true });
      }
    });
  }
