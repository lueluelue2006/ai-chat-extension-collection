(function () {
  ...
  window.chatGptNavDebug = {
    ...
    checkOverlap: () => {
      ...
      console.log(`键盘事件已绑定: ${readRuntimeFlag('__quicknavErnieKeysBoundV1', '__cgptKeysBound')}`);
      ...
      return { panels: panels.length, styles: styles.length, keysBound: readRuntimeFlag('__quicknavErnieKeysBoundV1', '__cgptKeysBound'), booting: __cgptBooting };
    },
    testObserver: () => {
  ...
  function detectUrlChange(payload = null) {
    ...
    if (nextHref !== currentUrl) {
      if (DEBUG || window.DEBUG_TEMP) console.log('ChatGPT Navigation: URL变化，清理旧实例', currentUrl, '->', nextHref);
      currentUrl = nextHref;
      ...
      writeRuntimeFlag('__quicknavErnieKeysBoundV1', '__cgptKeysBound', false);
      lastTurnCount = 0;
    ...
    try {
      if (QUICKNAV_ROUTE_WATCH && typeof QUICKNAV_ROUTE_WATCH.installBridgeFirstWatcher === 'function') {
        const installed = QUICKNAV_ROUTE_WATCH.installBridgeFirstWatcher({
          target: window,
          unsubKey: '__quicknavErnieRouteUnsubV1',
          pollKey: '__quicknavErnieRoutePollV1',
          pollMs: 1200,
  ...
  const ERNIE_THINKING_FINISHED_MARKERS = [
    'Finished thinking.',
    'Finished thinking',
    '思考结束',
    '思考完毕',
  ...
  const ERNIE_THINKING_FINISHED_RE = /Finished\s*thinking\.?|思考结束|思考完毕|推理结束|已完成思考/gi;
  const ERNIE_THINKING_ONLY_RE = /^(?:thinking|reasoning|analysis|thought(?:\s*process)?|thinking\s*user|思考(?:中|过程)?|正在思考|推理(?:中|过程)?|分析中|深度思考)(?:[\s.·•\-–—:：\]]|$)+/i;
  const ERNIE_THINKING_HINT_RE = /(thinking|reasoning|analysis|thinking\s*user|思考|推理|分析)/i;
  const ERNIE_USER_ID_HINT_RE = /\buser\d{6,}\b/i;
  const ERNIE_THINKING_COMPLETE_HINT_RE = /thinking\s*complete|思考(?:完成|完毕)/i;
  const ERNIE_THINKING_CONTAINER_SELECTOR =
    '[class*="thinking"],[class*="reason"],[class*="thought"],[class*="analysis"],[data-testid*="thinking"],[data-testid*="reasoning"],[aria-label*="思考"],[aria-label*="推理"]';

  function normalizeErniePreviewText(text, cap = 600) {
    const cleaned = String(text || '').replace(/\s+/g, ' ').trim();
    if (!cleaned) return '';
    return cleaned.length > cap ? cleaned.slice(0, cap) : cleaned;
  }

  function stripErnieThinkingLead(raw) {
    let text = String(raw || '');
    if (!text) return '';
    text = text.replace(/^[\s\u200b]+/, '');
    text = text.replace(/^[\s.·•\-–—:：]+/, '');
    text = text.replace(ERNIE_THINKING_ONLY_RE, '');
    text = text.replace(/^[\s.·•\-–—:：]+/, '');
    return text;
  }

  function isErnieTransientThinkingText(text, { hasFinishedMarker = false } = {}) {
    const normalized = normalizeErniePreviewText(text);
    if (!normalized || normalized === '...') return true;
    if (hasFinishedMarker) return false;
    if (ERNIE_THINKING_ONLY_RE.test(normalized)) return true;
    if (ERNIE_USER_ID_HINT_RE.test(normalized)) return true;
    if (ERNIE_THINKING_COMPLETE_HINT_RE.test(normalized)) return true;
    if (normalized.length <= 80 && ERNIE_THINKING_HINT_RE.test(normalized) && !/[。！？.!?]/.test(normalized)) return true;
    return false;
  }

  function parseErnieAssistantPreviewText(raw) {
    ...
    let bestLen = 0;
    try {
      const finishedRe = new RegExp(ERNIE_THINKING_FINISHED_RE.source, 'gi');
      let match = null;
      while ((match = finishedRe.exec(source))) {
        const idx = Number(match.index);
        const len = Number(match[0] ? String(match[0]).length : 0);
        if (idx >= 0 && len > 0 && idx >= bestIdx) {
          bestIdx = idx;
          bestLen = len;
        }
      }
    } catch {}
    for (const marker of ERNIE_THINKING_FINISHED_MARKERS) {
    ...
    const hasFinishedMarker = bestIdx >= 0;
    const afterMarker = hasFinishedMarker ? source.slice(bestIdx + bestLen) : source;
    const cleaned = stripErnieThinkingLead(afterMarker);
    const normalized = normalizeErniePreviewText(cleaned);
    if (normalized) {
      return { text: normalized, transient: isErnieTransientThinkingText(normalized, { hasFinishedMarker }) };
    }

    const fallback = normalizeErniePreviewText(stripErnieThinkingLead(source));
    if (fallback) {
      return { text: fallback, transient: isErnieTransientThinkingText(fallback, { hasFinishedMarker: false }) };
    }
    return { text: '...', transient: true };
  }

  function collectErnieAssistantCandidateTexts(el) {
    const out = [];
    const seen = new Set();
    const push = (raw) => {
    ...
    const addFromSelector = (selector, { skipThinkingAncestor = false } = {}) => {
      ...
      for (const node of nodes) {
        if (!node) continue;
        if (skipThinkingAncestor && typeof node.closest === 'function') {
          try {
            if (node.closest(ERNIE_THINKING_CONTAINER_SELECTOR)) continue;
          } catch {}
        }
    ...
    // 优先完整容器：在 Ernie 中 final answer 常与 thinking 共卡片，完整文本更容易命中“Finished thinking”分界。
    push(el && el.textContent ? el.textContent : '');
    addFromSelector('[class*="answerText__"], [class*="answerContent__"], [class*="answerContainer__"], [class*="contentWrap__"]', { skipThinkingAncestor: true });
    addFromSelector('.markdown, .prose, [data-message-content-part], p', { skipThinkingAncestor: true });
    addFromSelector('[class*="answerBox__"]', { skipThinkingAncestor: true });
    return out;
  }

  function getErnieAssistantPreview(el) {
    if (!el) return { text: '', transient: true };
    const candidates = collectErnieAssistantCandidateTexts(el);
    let fallback = null;
    for (const raw of candidates) {
      const result = parseErnieAssistantPreviewText(raw);
  ...
  function buildIndex(turnsOverride) {
    ...
    for (let i = 0; i < turns.length; i++) {
      ...
      if (!preview || shouldRecalcPreview) {
        ...
        if (!block) block = el;
        if (isAssistant) {
          const assistantPreview = getErnieAssistantPreview(el || block);
...
`;
  ...
  function wirePanel(ui) {
    ...
    // 键盘事件只绑定一次：避免重复绑定
    if (!readRuntimeFlag('__quicknavErnieKeysBoundV1', '__cgptKeysBound')) {
      const onKeydown = (e) => {
        const t = e.target;
      ...
      writeRuntimeFlag('__quicknavErnieKeysBoundV1', '__cgptKeysBound', true);
    ...
    const mo = new MutationObserver((muts) => {
      ...
      for (const mut of muts) {
        ...
        if (
          ...
          t.closest('.dialogue_card_item') ||
          t.closest('#card_list_id') ||
          t.closest('#DIALOGUE_CONTAINER_ID') ||
          t.closest('[class*="dialogueCard"]') ||
          t.closest('[class*="mdRenderContainer__"]') ||
          t.closest('[class*="questionText__"]') ||
          t.closest('[class*="roleUser__"]') ||
          t.closest('[class*="roleSystem__"]') ||
  ...
  function bindScrollLockUserIntents() {
    if (readRuntimeFlag('__quicknavErnieScrollLockUserIntentsBound', '__cgptScrollLockUserIntentsBound')) return;
    writeRuntimeFlag('__quicknavErnieScrollLockUserIntentsBound', '__cgptScrollLockUserIntentsBound', true);
  ...
  function installScrollGuards() {
    if (readRuntimeFlag('__quicknavErnieScrollGuardsInstalled', '__cgptScrollGuardsInstalled')) return;
    writeRuntimeFlag('__quicknavErnieScrollGuardsInstalled', '__cgptScrollGuardsInstalled', true);
  ...
  function initScrollLock(ui) {
    ...
    if (!readRuntimeFlag('__quicknavErnieScrollLockBound', '__cgptScrollLockBound')) {
      window.addEventListener('resize', () => { if (scrollLockEnabled) ensureScrollLockBindings(); }, { passive: true });
      writeRuntimeFlag('__quicknavErnieScrollLockBound', '__cgptScrollLockBound', true);
    }
  ...
  function mutationTouchesConversation(node) {
    ...
    if (node.matches('.dialogue_card_item, #card_list_id, #DIALOGUE_CONTAINER_ID, [class*="dialogueCard"], [class*="mdRenderContainer__"], [class*="questionText__"], [class*="roleUser__"], [class*="roleSystem__"]')) return true;
    if (node.matches('.markdown, .prose, article')) return true;
    if (node.querySelector?.('[data-message-author-role], [data-message-id], .dialogue_card_item, #card_list_id, #DIALOGUE_CONTAINER_ID, [class*="dialogueCard"], [class*="mdRenderContainer__"], [class*="questionText__"], [class*="roleUser__"], [class*="roleSystem__"], .markdown, .prose')) return true;
    return false;
  }
  ...
  function bindAltPin(ui) {
    if (readRuntimeFlag('__quicknavErniePinBound', '__cgptPinBound')) return;
    ...
    writeRuntimeFlag('__quicknavErniePinBound', '__cgptPinBound', true);
  }
  ...
  function watchSendEvents(ui) {
    ...
    const isErnieSendControl = (node) => {
      try {
        return !!(node && node.closest && node.closest('[class*="send__"], [class*="sendInner__"], [class*="sendBtn"]'));
      } catch {
        return false;
      }
    };
    const isErnieComposerTarget = (node) => {
      try {
        return !!(node && node.closest && node.closest('[role="textbox"], [class*="editable__"]'));
      } catch {
        return false;
      }
    };
    ...
    document.addEventListener('click', (e) => {
      if ((e.target && e.target.closest && e.target.closest('[data-testid="send-button"]')) || isErnieSendControl(e.target)) {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      const isErnieComposer = isErnieComposerTarget(t);
      if ((isTextarea || isErnieComposer) && e.key === 'Enter' && !e.shiftKey && !e.altKey && !e.ctrlKey && !e.metaKey) {
        // ERNIE 的发送路径可能走纯 Enter；提前拉起滚动锁守护与突发刷新，避免“延迟跳转”。
        armScrollLockGuard(2200);
        startBurstRefresh(ui);
        return;
      }
