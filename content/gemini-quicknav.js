(function () {
  ...
  // === Gemini (open) shadow DOM helpers ===
  const GEMINI_HOST_SELECTOR = 'ucs-standalone-app';
  let __geminiScrollerCache = null;

  function getGeminiHost() { return document.querySelector(GEMINI_HOST_SELECTOR); }
  function getGeminiRoot() { return getGeminiHost()?.shadowRoot || null; }
  ...
  function getGeminiScroller() {
    if (__geminiScrollerCache && __geminiScrollerCache.isConnected) return __geminiScrollerCache;
    const root = getGeminiRoot();
    if (!root) return null;
    const scroller = deepQueryFirst(root, 'div.chat-mode-scroller') || deepQueryFirst(root, 'div.chat-mode-scroller.tile-content');
    __geminiScrollerCache = scroller || null;
    return __geminiScrollerCache;
  }

  const GEMINI_NAV_DEBUG_KEY = 'geminiNavDebug';
  const GEMINI_NAV_DEBUG_LEGACY_KEY = 'chatGptNavDebug';
  const GEMINI_KEYS_BOUND_KEY = '__quicknavGeminiKeysBoundV1';
  const GEMINI_KEYS_BOUND_LEGACY_KEY = '__cgptKeysBound';
  const GEMINI_SCROLL_LOCK_USER_INTENTS_BOUND_KEY = '__quicknavGeminiScrollLockUserIntentsBoundV1';
  const GEMINI_SCROLL_LOCK_USER_INTENTS_BOUND_LEGACY_KEY = '__cgptScrollLockUserIntentsBound';
  const GEMINI_SCROLL_GUARDS_INSTALLED_KEY = '__quicknavGeminiScrollGuardsInstalledV1';
  const GEMINI_SCROLL_GUARDS_INSTALLED_LEGACY_KEY = '__cgptScrollGuardsInstalled';
  const GEMINI_SCROLL_LOCK_BOUND_KEY = '__quicknavGeminiScrollLockBoundV1';
  const GEMINI_SCROLL_LOCK_BOUND_LEGACY_KEY = '__cgptScrollLockBound';
  const GEMINI_PIN_BOUND_KEY = '__quicknavGeminiPinBoundV1';
  const GEMINI_PIN_BOUND_LEGACY_KEY = '__cgptPinBound';
  ...
  function writeRuntimeFlag(primaryKey, legacyKey, value) {
    const next = !!value;
    try { window[primaryKey] = next; } catch {}
    try { window[legacyKey] = next; } catch {}
    return next;
  }

  function isGeminiKeysBound() {
    return readRuntimeFlag(GEMINI_KEYS_BOUND_KEY, GEMINI_KEYS_BOUND_LEGACY_KEY);
  }

  function setGeminiKeysBound(value) {
    return writeRuntimeFlag(GEMINI_KEYS_BOUND_KEY, GEMINI_KEYS_BOUND_LEGACY_KEY, value);
  }

  function isGeminiScrollLockUserIntentsBound() {
    return readRuntimeFlag(GEMINI_SCROLL_LOCK_USER_INTENTS_BOUND_KEY, GEMINI_SCROLL_LOCK_USER_INTENTS_BOUND_LEGACY_KEY);
  }

  function setGeminiScrollLockUserIntentsBound(value) {
    return writeRuntimeFlag(GEMINI_SCROLL_LOCK_USER_INTENTS_BOUND_KEY, GEMINI_SCROLL_LOCK_USER_INTENTS_BOUND_LEGACY_KEY, value);
  }

  function isGeminiScrollGuardsInstalled() {
    return readRuntimeFlag(GEMINI_SCROLL_GUARDS_INSTALLED_KEY, GEMINI_SCROLL_GUARDS_INSTALLED_LEGACY_KEY);
  }

  function setGeminiScrollGuardsInstalled(value) {
    return writeRuntimeFlag(GEMINI_SCROLL_GUARDS_INSTALLED_KEY, GEMINI_SCROLL_GUARDS_INSTALLED_LEGACY_KEY, value);
  }

  function isGeminiScrollLockBound() {
    return readRuntimeFlag(GEMINI_SCROLL_LOCK_BOUND_KEY, GEMINI_SCROLL_LOCK_BOUND_LEGACY_KEY);
  }

  function setGeminiScrollLockBound(value) {
    return writeRuntimeFlag(GEMINI_SCROLL_LOCK_BOUND_KEY, GEMINI_SCROLL_LOCK_BOUND_LEGACY_KEY, value);
  }

  function isGeminiPinBound() {
    return readRuntimeFlag(GEMINI_PIN_BOUND_KEY, GEMINI_PIN_BOUND_LEGACY_KEY);
  }

  function setGeminiPinBound(value) {
    return writeRuntimeFlag(GEMINI_PIN_BOUND_KEY, GEMINI_PIN_BOUND_LEGACY_KEY, value);
  }

  function installGeminiNavDebugApi(debugApi) {
    try { window[GEMINI_NAV_DEBUG_KEY] = debugApi; } catch {}
    try { window[GEMINI_NAV_DEBUG_LEGACY_KEY] = debugApi; } catch {}
  }

  // 全局调试函数，用户可在控制台调用
  const geminiNavDebugApi = {
    forceRefresh: () => {
    ...
    checkOverlap: () => {
      ...
      console.log(`键盘事件已绑定: ${isGeminiKeysBound()}`);
      console.log(`正在启动中: ${__geminiBooting}`);
      if (panels.length > 1) {
      ...
      return { panels: panels.length, styles: styles.length, keysBound: isGeminiKeysBound(), booting: __geminiBooting };
    },
    testObserver: () => {
  ...
  installGeminiNavDebugApi(geminiNavDebugApi);
  ...
  registerMenuCommand("清理过期检查点（31天）", cleanupExpiredCheckpoints, { moduleId: 'quicknav' });
  ...
  function init() {
    ...
    const boot = () => {
      ...
      if (document.getElementById('cgpt-compact-nav')) {
        if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: 面板已存在，跳过创建');
        return;
      }
      if (__geminiBooting) {
        if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: 正在启动中，跳过重复创建');
        return;
      }

      __geminiBooting = true;
      try {
        if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: 开始创建面板');
        ...
        scheduleRefresh(ui);
        if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: 面板创建完成');
      } finally {
        __geminiBooting = false;
      }
    };
    const tryBoot = () => {
  ...
  function detectUrlChange() {
    if (location.href !== currentUrl) {
      if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: URL变化，清理旧实例', currentUrl, '->', location.href);
      ...
      if (oldNav) {
        if (oldNav._ui) {
          ...
          if (oldNav._ui._forceRefreshTimer) {
            ...
            if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: 已清理定时器');
          }
          // 断开MutationObserver
          if (oldNav._ui._mo) {
            try {
              oldNav._ui._mo.disconnect();
              if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: 已断开MutationObserver');
            } catch (e) {
              if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: 断开MutationObserver失败', e);
            }
          }
        ...
        oldNav.remove();
        if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: 已移除旧面板');
      }
      // 重置"正在启动"标志，避免新页面被卡住
      __geminiBooting = false;
      // 重置键盘事件绑定标志，允许新页面重新绑定
      setGeminiKeysBound(false);
      lastTurnCount = 0;
  ...
  function qsTurns() {
    ...
    if (!scroller) return [];
    const turns = deepQueryAll(scroller, 'div.turn');
  ...
  function extractGeminiTurnParts(turnEl) {
    const docs = deepQueryAll(turnEl, '.markdown-document');
    let qDoc = null;
    let aDoc = null;
    for (const doc of docs) {
      const text = (doc.textContent || '').replace(/\s+/g, ' ').trim();
      if (!text) continue;
      const k = classifyMarkdownDoc(doc);
      if (k.isQuestion && !qDoc) qDoc = doc;
      if (k.isAnswer && !aDoc) aDoc = doc;
      if (qDoc && aDoc) break;
    }

    // 兜底：找一个可能的答案容器（在生成中也可能存在）
    const answerFallback =
      aDoc ||
      deepQueryFirst(turnEl, 'div.summary-contents') ||
      deepQueryFirst(turnEl, 'div.summary') ||
      deepQueryFirst(turnEl, 'ucs-summary') ||
      deepQueryFirst(turnEl, 'ucs-response-markdown');

    const questionText = qDoc ? getTextPreview(qDoc) : '';
    const answerText = aDoc ? getTextPreview(aDoc) : (answerFallback ? getTextPreview(answerFallback) : '');

    return { qDoc, aDoc: aDoc || null, answerFallback: answerFallback || null, questionText, answerText };
  }
...
`;
      ...
      if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: 已创建样式');
    } else {
      if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: 样式已存在，跳过创建');
    }

    // 启动前清理多余面板（保险丝）
    const existingPanels = document.querySelectorAll('#cgpt-compact-nav');
    if (existingPanels.length > 0) {
      if (DEBUG || window.DEBUG_TEMP) console.log(`Gemini Navigation: 发现 ${existingPanels.length} 个已存在的面板，清理中...`);
      existingPanels.forEach((panel, index) => {
        if (index > 0) { // 保留第一个，删除其他
          panel.remove();
          if (DEBUG || window.DEBUG_TEMP) console.log(`Gemini Navigation: 已删除重复面板 ${index}`);
        }
      });
      // 如果已经有面板存在，直接返回现有的
      if (existingPanels.length > 0) {
        ...
        if (existingNav._ui) {
          if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: 返回已存在的面板');
    ...
    try {
      ...
      if (DEBUG || window.DEBUG_TEMP) console.error('Gemini Navigation: 布局管理器初始化失败', err);
    }
    enableDrag(nav, {
  ...
  function createLayoutManager(nav) {
    ...
    function scheduleEvaluation(reason) {
      ...
      state.rafId = requestAnimationFrame(() => {
        state.rafId = 0;
        state.pendingEval = false;
        try { evaluateNow(reason); } catch (err) { if (DEBUG || window.DEBUG_TEMP) console.error('Gemini Navigation layout evaluate error:', err); }
      });
    }
  ...
  function refreshIndex(ui, { soft = false, force = false } = {}) {
    ...
    if (DEBUG) console.log('Gemini Navigation: turns', next.length, '(含📌)');
  ...
  function findTurnAnchor(root) {
    ...
    const selectors = [
      // Gemini Enterprise
      '.question-wrapper .markdown-document',
      '.summary-contents .markdown-document',
      '.markdown-document',
  ...
  function wirePanel(ui) {
    ...
    if (refreshBtn) {
      refreshBtn.addEventListener('click', (e) => {
        if (e.shiftKey) {
          // Shift+点击 = 强制重新扫描
          if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: 强制重新扫描 (清除缓存选择器)');
      ...
      refreshBtn.addEventListener('contextmenu', (e) => {
        e.preventDefault();
        if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: 右键强制重新扫描');
        TURN_SELECTOR = null;
        const originalBg = refreshBtn.style.background;
        const originalColor = refreshBtn.style.color;
        refreshBtn.style.background = 'var(--cgpt-nav-accent-subtle)';
    ...
    if (!isGeminiKeysBound()) {
      ...
      setGeminiKeysBound(true);
      if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: 已绑定键盘事件');
    } else {
      if (DEBUG || window.DEBUG_TEMP) console.log('Gemini Navigation: 键盘事件已存在，跳过绑定');
    }
  }
  ...
  function postScrollLockStateToMainWorld() {
    try {
      window.postMessage({ __quicknav: 1, type: 'AISHORTCUTS_SCROLLLOCK_STATE', enabled: !!scrollLockEnabled }, '*');
    } catch {}
  }
  ...
  function postScrollLockBaselineToMainWorld(top, force = false) {
    try {
      ...
      window.postMessage({ __quicknav: 1, type: 'AISHORTCUTS_SCROLLLOCK_BASELINE', top: px }, '*');
    } catch {}
  }

  function postScrollLockAllowToMainWorld(ms) {
    try {
      window.postMessage({ __quicknav: 1, type: 'AISHORTCUTS_SCROLLLOCK_ALLOW', ms: Number(ms) || 0 }, '*');
    } catch {}
  }
  ...
  function bindScrollLockUserIntents() {
    if (isGeminiScrollLockUserIntentsBound()) return;
    setGeminiScrollLockUserIntentsBound(true);
  ...
  function installScrollGuards() {
    if (isGeminiScrollGuardsInstalled()) return;
    setGeminiScrollGuardsInstalled(true);
  ...
  function findTurnFromNode(node) {
    ...
    if (el) return el;
    // 兜底：尝试已知选择器
    el = closestCrossShadow(node, 'div.turn') ||
