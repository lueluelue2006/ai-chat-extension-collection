(function () {
  ...
  const KIMI_KEYS_BOUND_KEY = '__quicknavKimiKeysBoundV1';
  const KIMI_KEYS_BOUND_LEGACY_KEY = '__cgptKeysBound';
  const KIMI_SCROLL_LOCK_USER_INTENTS_BOUND_KEY = '__quicknavKimiScrollLockUserIntentsBound';
  const KIMI_SCROLL_LOCK_USER_INTENTS_BOUND_LEGACY_KEY = '__cgptScrollLockUserIntentsBound';
  const KIMI_SCROLL_GUARDS_INSTALLED_KEY = '__quicknavKimiScrollGuardsInstalled';
  const KIMI_SCROLL_GUARDS_INSTALLED_LEGACY_KEY = '__cgptScrollGuardsInstalled';
  const KIMI_SCROLL_LOCK_BOUND_KEY = '__quicknavKimiScrollLockBound';
  const KIMI_SCROLL_LOCK_BOUND_LEGACY_KEY = '__cgptScrollLockBound';
  const KIMI_PIN_BOUND_KEY = '__quicknavKimiPinBound';
  const KIMI_PIN_BOUND_LEGACY_KEY = '__cgptPinBound';
  ...
  function isKimiKeysBound() {
    return readRuntimeFlag(KIMI_KEYS_BOUND_KEY, KIMI_KEYS_BOUND_LEGACY_KEY);
  }

  function setKimiKeysBound(value) {
    return writeRuntimeFlag(KIMI_KEYS_BOUND_KEY, KIMI_KEYS_BOUND_LEGACY_KEY, value);
  }

  function isKimiScrollLockUserIntentsBound() {
    return readRuntimeFlag(KIMI_SCROLL_LOCK_USER_INTENTS_BOUND_KEY, KIMI_SCROLL_LOCK_USER_INTENTS_BOUND_LEGACY_KEY);
  }

  function setKimiScrollLockUserIntentsBound(value) {
    return writeRuntimeFlag(KIMI_SCROLL_LOCK_USER_INTENTS_BOUND_KEY, KIMI_SCROLL_LOCK_USER_INTENTS_BOUND_LEGACY_KEY, value);
  }

  function isKimiScrollGuardsInstalled() {
    return readRuntimeFlag(KIMI_SCROLL_GUARDS_INSTALLED_KEY, KIMI_SCROLL_GUARDS_INSTALLED_LEGACY_KEY);
  }

  function setKimiScrollGuardsInstalled(value) {
    return writeRuntimeFlag(KIMI_SCROLL_GUARDS_INSTALLED_KEY, KIMI_SCROLL_GUARDS_INSTALLED_LEGACY_KEY, value);
  }

  function isKimiScrollLockBound() {
    return readRuntimeFlag(KIMI_SCROLL_LOCK_BOUND_KEY, KIMI_SCROLL_LOCK_BOUND_LEGACY_KEY);
  }

  function setKimiScrollLockBound(value) {
    return writeRuntimeFlag(KIMI_SCROLL_LOCK_BOUND_KEY, KIMI_SCROLL_LOCK_BOUND_LEGACY_KEY, value);
  }

  function isKimiPinBound() {
    return readRuntimeFlag(KIMI_PIN_BOUND_KEY, KIMI_PIN_BOUND_LEGACY_KEY);
  }

  function setKimiPinBound(value) {
    return writeRuntimeFlag(KIMI_PIN_BOUND_KEY, KIMI_PIN_BOUND_LEGACY_KEY, value);
  }

  // 全局调试函数，用户可在控制台调用
  const kimiQuicknavDebugApi = {
    forceRefresh: () => {
      console.log('Kimi QuickNav: 手动强制刷新');
      TURN_SELECTOR = null;
    ...
    checkOverlap: () => {
      ...
      console.log(`键盘事件已绑定: ${isKimiKeysBound()}`);
      console.log(`正在启动中: ${kimiQuicknavBooting}`);
      if (panels.length > 1) {
      ...
      return { panels: panels.length, styles: styles.length, keysBound: isKimiKeysBound(), booting: kimiQuicknavBooting };
    },
    testObserver: () => {
  ...
  window.kimiQuicknavDebug = kimiQuicknavDebugApi;
  window.kimiNavDebug = kimiQuicknavDebugApi;
  window.chatGptNavDebug = kimiQuicknavDebugApi; // backward compatible alias
  ...
  function init() {
    ...
    const boot = () => {
      ...
      if (document.getElementById('cgpt-compact-nav')) {
        if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 面板已存在，跳过创建');
        return;
      }
      if (kimiQuicknavBooting) {
        if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 正在启动中，跳过重复创建');
        return;
      }

      kimiQuicknavBooting = true;
      try {
        if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 开始创建面板');
        ...
        scheduleRefresh(ui);
        if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 面板创建完成');
      } finally {
        kimiQuicknavBooting = false;
      }
    };
  ...
  function detectUrlChange() {
    if (location.href !== currentUrl) {
      if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: URL变化，清理旧实例', currentUrl, '->', location.href);
      ...
      if (oldNav) {
        if (oldNav._ui) {
          ...
          if (oldNav._ui._forceRefreshTimer) {
            ...
            if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 已清理定时器');
          }
          // 断开MutationObserver
          if (oldNav._ui._mo) {
            try {
              oldNav._ui._mo.disconnect();
              if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 已断开MutationObserver');
            } catch (e) {
              if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 断开MutationObserver失败', e);
            }
          }
        ...
        oldNav.remove();
        if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 已移除旧面板');
      }
      // 重置"正在启动"标志，避免新页面被卡住
      kimiQuicknavBooting = false;
      lastTurnCount = 0;
    ...
    try {
      if (QUICKNAV_ROUTE_WATCH && typeof QUICKNAV_ROUTE_WATCH.installBridgeFirstWatcher === 'function') {
        const installed = QUICKNAV_ROUTE_WATCH.installBridgeFirstWatcher({
          target: window,
          unsubKey: '__quicknavKimiRouteUnsubV1',
          pollKey: '__quicknavKimiRoutePollV1',
          pollMs: 1200,
  ...
  function qsTurns(root = document) {
    ...
    if (DEBUG || window.DEBUG_TEMP) {
      console.log('Kimi QuickNav Debug: 检测对话选择器');
    ...
    for (const selector of selectors) {
      ...
      if (els.length) {
        ...
        if (DEBUG || window.DEBUG_TEMP) console.log(`Kimi QuickNav: 使用选择器 ${selector}, 找到 ${els.length} 个对话`);
  ...
  function buildIndex(turnsOverride) {
    ...
    turnIdToPos.clear();
    if (!turns.length) {
      if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 没有找到任何对话元素');
      return [];
    }

    if (DEBUG) console.log(`Kimi QuickNav: 开始分析 ${turns.length} 个对话元素`);
    ...
    for (let i = 0; i < turns.length; i++) {
      ...
      if (DEBUG && i < 3) {
        console.log(`Kimi QuickNav Debug - 元素 ${i}:`, {
          element: el,
          testId: attrTestId,
          isUser,
          isAssistant,
      ...
      if (!isUser && !isAssistant) {
        if (DEBUG && i < 5) console.log(`Kimi QuickNav: 元素 ${i} 角色识别失败`);
        continue;
      }
      ...
      if (!preview) {
        if (DEBUG && i < 5) console.log(`Kimi QuickNav: 元素 ${i} 无法提取预览文本`);
        continue;
      }
    ...
    if (DEBUG) console.log(`Kimi QuickNav: 成功识别 ${list.length} 个对话 (用户: ${u}, 助手: ${a})`);
    return list;
  }
...
`;
      ...
      if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 已创建样式');
    } else {
      if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 样式已存在，跳过创建');
    }

    // 启动前清理多余面板（保险丝）
    const existingPanels = document.querySelectorAll('#cgpt-compact-nav');
    if (existingPanels.length > 0) {
      if (DEBUG || window.DEBUG_TEMP) console.log(`Kimi QuickNav: 发现 ${existingPanels.length} 个已存在的面板，清理中...`);
      existingPanels.forEach((panel, index) => {
        if (index > 0) { // 保留第一个，删除其他
          panel.remove();
          if (DEBUG || window.DEBUG_TEMP) console.log(`Kimi QuickNav: 已删除重复面板 ${index}`);
        }
      });
      // 如果已经有面板存在，直接返回现有的
      if (existingPanels.length > 0) {
        ...
        if (existingNav._ui) {
          if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 返回已存在的面板');
    ...
    try {
      ...
      if (DEBUG || window.DEBUG_TEMP) console.error('Kimi QuickNav: 布局管理器初始化失败', err);
    }
    enableDrag(nav, {
  ...
  function createLayoutManager(nav) {
    ...
    function scheduleEvaluation(reason) {
      ...
      state.rafId = requestAnimationFrame(() => {
        state.rafId = 0;
        state.pendingEval = false;
        try { evaluateNow(reason); } catch (err) { if (DEBUG || window.DEBUG_TEMP) console.error('Kimi QuickNav layout evaluate error:', err); }
      });
    }
  ...
  function refreshIndex(ui, { soft = false, force = false } = {}) {
    ...
    if (DEBUG) console.log('Kimi QuickNav: turns', next.length, '(含📌)');
  ...
  function wirePanel(ui) {
    ...
    if (refreshBtn) {
      refreshBtn.addEventListener('click', (e) => {
        if (e.shiftKey) {
          // Shift+点击 = 强制重新扫描
          if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 强制重新扫描 (清除缓存选择器)');
      ...
      refreshBtn.addEventListener('contextmenu', (e) => {
        e.preventDefault();
        if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 右键强制重新扫描');
        TURN_SELECTOR = null;
        const originalBg = refreshBtn.style.background;
        const originalColor = refreshBtn.style.color;
        refreshBtn.style.background = 'var(--cgpt-nav-accent-subtle)';
    ...
    if (!isKimiKeysBound()) {
      ...
      setKimiKeysBound(true);
      if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 已绑定键盘事件');
    } else {
      if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 键盘事件已存在，跳过绑定');
    }
  }
  ...
  function getTurnsContainer() {
    ...
    if (!nodes.length) {
      ...
      if (DEBUG && potentialContainers.length > 1) {
        console.log('Kimi QuickNav: 没有找到对话，使用备用容器:', potentialContainers[0]);
      }
    ...
    while (a) {
      if (nodes.every(n => a.contains(n))) {
        if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 对话容器:', a);
        return a;
      }
      a = a.parentElement;
    }
  ...
  function bindScrollLockUserIntents() {
    if (isKimiScrollLockUserIntentsBound()) return;
    setKimiScrollLockUserIntentsBound(true);
  ...
  function installScrollGuards() {
    if (isKimiScrollGuardsInstalled()) return;
    setKimiScrollGuardsInstalled(true);
  ...
  function watchSendEvents(ui) {
    if (window.__quicknavKimiSendEventsBoundV1) return;
    window.__quicknavKimiSendEventsBoundV1 = true;
    ...
    const hasKimiComposerMarker = (scope) => {
      if (!(scope instanceof Element)) return false;
      try {
        if (scope.matches('.chat-editor, .chat-action')) return true;
      } catch {}
      try {
        const hasEditor = !!scope.querySelector?.('.chat-input-editor[contenteditable="true"][role="textbox"]');
        const hasSendControl = !!scope.querySelector?.('.send-button-container');
        if (hasEditor && hasSendControl) return true;
      } catch {}
      return false;
    };

    const getKimiComposerScopeFrom = (target) => {
      const el = asElement(target);
      if (!el) return null;

      try {
        const byComposer = el.closest('.chat-editor, .chat-action');
        if (byComposer && hasKimiComposerMarker(byComposer)) return byComposer;
      } catch {}

      try {
        const byEditor = el.closest('.chat-input-editor[contenteditable="true"][role="textbox"]');
        if (byEditor) {
          const byParent = byEditor.closest('.chat-editor, .chat-action');
          if (byParent && hasKimiComposerMarker(byParent)) return byParent;
          return byEditor.parentElement || null;
        }
      } catch {}
    ...
    const isComposerScope = (scope) => hasKimiComposerMarker(scope);

    const isSendActionTarget = (target) => {
      if (isInsideQuickNav(target)) return false;
      const el = asElement(target);
      if (!el) return false;
      const sendButton = el.closest('.send-button-container');
      if (!sendButton) return false;
      if (!getKimiComposerScopeFrom(sendButton)) return false;
      if (isStopLikeControl(sendButton)) return false;
      return true;
    };

    const isKimiPromptTarget = (target) => {
      if (isInsideQuickNav(target)) return false;
      const el = asElement(target);
      if (!el) return false;
      try {
        const editor = el.closest('.chat-input-editor[contenteditable="true"][role="textbox"]');
        if (!editor) return false;
        return !!getKimiComposerScopeFrom(editor);
      } catch {
        return false;
      }
    };
    ...
    document.addEventListener('click', (e) => {
      ...
      if (!currentUi) return;
      if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 检测到发送按钮点击，启动突发刷新');
      armScrollLockGuard(2200);
    ...
    document.addEventListener('submit', (e) => {
      ...
      const scope = getKimiComposerScopeFrom(form) || form;
      const stopLike = queryStopLikeControl(scope) || queryStopLikeControl(document);
      if (stopLike) return;
      const currentUi = getUi();
      if (!currentUi) return;
      if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 检测到表单提交，启动突发刷新');
      armScrollLockGuard(2200);
      startBurstRefresh(currentUi);
    }, true);
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (!isKimiPromptTarget(e.target)) return;
      const scope = getKimiComposerScopeFrom(e.target) || document.querySelector('.chat-editor, .chat-action') || document;
      const stopLike = queryStopLikeControl(scope) || queryStopLikeControl(document);
      if (stopLike) return;
      const currentUi = getUi();
      if (!currentUi) return;
      if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 检测到快捷键发送，启动突发刷新');
      armScrollLockGuard(2200);
      startBurstRefresh(currentUi);
    }, true);
    ...
    document.addEventListener('visibilitychange', () => {
      if (!document.hidden) {
        const currentUi = getUi();
        if (!currentUi) return;
        if (DEBUG || window.DEBUG_TEMP) console.log('Kimi QuickNav: 页面重新可见，强制刷新');
