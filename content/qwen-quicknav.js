(function () {
  ...
  // 全局调试函数，用户可在控制台调用
  const qwenQuicknavDebugApi = {
    forceRefresh: () => {
      console.log('Qwen QuickNav: 手动强制刷新');
      TURN_SELECTOR = null;
    ...
    checkOverlap: () => {
      ...
      console.log(`键盘事件已绑定: ${isCompatBoolFlagSet('__qwenQuicknavKeysBound', '__cgptKeysBound')}`);
      console.log(`正在启动中: ${qwenQuicknavBooting}`);
      if (panels.length > 1) {
      ...
      return {
        ...
        keysBound: isCompatBoolFlagSet('__qwenQuicknavKeysBound', '__cgptKeysBound'),
        booting: qwenQuicknavBooting
      };
    },
    testObserver: () => {
  ...
  window.qwenQuicknavDebug = qwenQuicknavDebugApi;
  window.qwenNavDebug = qwenQuicknavDebugApi;
  window.chatGptNavDebug = qwenQuicknavDebugApi; // backward compatible alias
  ...
  let qwenQuicknavBooting = false;
  let refreshTimer = 0; // 新的尾随去抖定时器
  let lastStopCheckTs = 0;
  let lastHasStop = null;
  let qwenActiveLockController = null;
  let qwenRouteGateController = null;
  let qwenRouteGateSamplerTimer = 0;
  let qwenRouteGatePendingUi = null;
  let qwenRouteGateReleasedVersion = -1;
  let qwenRouteBaselineFingerprint = '';

  function getQwenQuicknavActiveLockController() {
    if (qwenActiveLockController) return qwenActiveLockController;
    try {
      const api = globalThis.__quicknavQwenActiveLock;
      const create = api && typeof api.createQwenQuicknavActiveLockController === 'function'
        ? api.createQwenQuicknavActiveLockController
        : null;
      if (create) qwenActiveLockController = create();
    } catch {}
    return qwenActiveLockController;
  }

  function getQwenQuicknavRouteGateController() {
    if (qwenRouteGateController) return qwenRouteGateController;
    try {
      const api = globalThis.__quicknavQwenRouteGate;
      const create = api && typeof api.createQwenQuicknavRouteGateController === 'function'
        ? api.createQwenQuicknavRouteGateController
        : null;
      if (create) qwenRouteGateController = create();
    } catch {}
    return qwenRouteGateController;
  }

  function syncLockedTurnPresence(nowMs = Date.now()) {
    const controller = getQwenQuicknavActiveLockController();
    if (!controller) return;
    if (typeof controller.getLockedId !== 'function' || typeof controller.onLockedIdResolved !== 'function') return;
    const lockedId = controller.getLockedId(nowMs);
    if (!lockedId) return;
    if (!document.getElementById(lockedId)) controller.onLockedIdResolved(false, nowMs);
  }

  function getTurnRoleHint(el) {
    if (!el || el.nodeType !== 1) return '';
    const attr = String(el.getAttribute('data-message-author-role') || '').trim();
    if (attr) return attr;
    if (el.classList && el.classList.contains('qwen-chat-message-user')) return 'user';
    if (el.classList && el.classList.contains('qwen-chat-message-assistant')) return 'assistant';
    return '';
  }

  function getTurnFingerprintToken(el) {
    if (!el || el.nodeType !== 1) return '';
    const key = getTurnKey(el);
    if (key) return `k:${String(key).slice(0, 96)}`;
    const role = getTurnRoleHint(el) || '?';
    const text = String(el.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 48);
    return `r:${role}|t:${text}`;
  }

  function sampleConversationFingerprint() {
    try {
      const turns = qsTurns();
      const count = turns.length;
      if (!count) return '';
      const first = getTurnFingerprintToken(turns[0]);
      const last = getTurnFingerprintToken(turns[count - 1]);
      return `n:${count}|f:${first}|l:${last}`;
    } catch {
      return '';
    }
  }

  function stopQwenRouteGateSampler() {
    if (!qwenRouteGateSamplerTimer) return;
    clearInterval(qwenRouteGateSamplerTimer);
    qwenRouteGateSamplerTimer = 0;
  }

  function isQwenRouteGatePending(nowMs = Date.now()) {
    const controller = getQwenQuicknavRouteGateController();
    if (!controller || typeof controller.shouldKeepOld !== 'function') return false;
    return !!controller.shouldKeepOld(nowMs);
  }

  function maybeSampleQwenRouteGate(nowMs = Date.now()) {
    const controller = getQwenQuicknavRouteGateController();
    if (!controller || typeof controller.onFingerprintSample !== 'function') {
      return !isQwenRouteGatePending(nowMs);
    }
    let routeVersion;
    if (typeof controller.getState === 'function') {
      try {
        routeVersion = Number(controller.getState(nowMs)?.routeVersion);
      } catch {}
    }
    const sampled = sampleConversationFingerprint();
    const fingerprint = sampled && sampled !== qwenRouteBaselineFingerprint ? sampled : '';
    const payload = { fingerprint, nowMs };
    if (Number.isFinite(routeVersion)) payload.routeVersion = routeVersion;
    controller.onFingerprintSample(payload);
    return !isQwenRouteGatePending(nowMs);
  }

  function flushQwenRouteGateReady(ui, nowMs = Date.now(), reason = '') {
    const controller = getQwenQuicknavRouteGateController();
    let routeVersion = -1;
    let pendingState = false;
    if (controller && typeof controller.getState === 'function') {
      try {
        const state = controller.getState(nowMs);
        routeVersion = Number(state?.routeVersion);
        pendingState = !!state?.pending;
      } catch {}
    }
    if (Number.isFinite(routeVersion) && routeVersion >= 0) {
      if (!pendingState && routeVersion === qwenRouteGateReleasedVersion) return;
      qwenRouteGateReleasedVersion = routeVersion;
    }
    stopQwenRouteGateSampler();
    qwenRouteGatePendingUi = null;
    qwenRouteBaselineFingerprint = '';

    TURN_SELECTOR = null;
    previewCache.clear();
    provisionalPreviewKeys.clear();
    hasProvisionalPreviews = false;
    roleCache.clear();
    turnIdToPos.clear();
    cachedTurns = [];
    lastDomTurnCount = 0;
    ...
    const targetUi = ui || document.getElementById('cgpt-compact-nav')?._ui || null;
    if (!targetUi) {
      setTimeout(init, 80);
      return;
    }
    if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 路由门就绪，强制刷新列表', reason || '');
    observeChat(targetUi);
    scheduleRefresh(targetUi, { force: true, bypassRouteGate: true });
  }

  function ensureQwenRouteGateSampler(ui) {
    if (ui) qwenRouteGatePendingUi = ui;
    if (qwenRouteGateSamplerTimer) return;
    qwenRouteGateSamplerTimer = setInterval(() => {
      const nowMs = Date.now();
      if (!isQwenRouteGatePending(nowMs)) {
        flushQwenRouteGateReady(qwenRouteGatePendingUi, nowMs, 'ready-or-timeout');
        return;
      }
      const targetUi = qwenRouteGatePendingUi || document.getElementById('cgpt-compact-nav')?._ui || null;
      const ready = maybeSampleQwenRouteGate(nowMs);
      if (ready) flushQwenRouteGateReady(targetUi, nowMs, 'sampler');
    }, 160);
  }

  function shouldSuppressRefreshForPendingRoute(ui) {
    const nowMs = Date.now();
    if (!isQwenRouteGatePending(nowMs)) {
      if (qwenRouteGatePendingUi) {
        flushQwenRouteGateReady(ui || qwenRouteGatePendingUi, nowMs, 'ready-check');
        return true;
      }
      return false;
    }
    if (ui) qwenRouteGatePendingUi = ui;
    if (maybeSampleQwenRouteGate(nowMs)) {
      flushQwenRouteGateReady(ui, nowMs, 'refresh');
    } else {
      ensureQwenRouteGateSampler(ui);
    }
    return true;
  }

  function beginRoutePendingTransition(nextHref, reason = 'url') {
    const href = String(nextHref || location.href || '').trim();
    if (!href || href === currentUrl) return false;
    qwenRouteBaselineFingerprint = sampleConversationFingerprint();
    const prevHref = currentUrl;
    currentUrl = href;

    const nowMs = Date.now();
    if (DEBUG || window.DEBUG_TEMP) {
      console.log('Qwen QuickNav: URL变化，进入待就绪状态', prevHref, '->', href, reason ? `(${reason})` : '');
    }

    const activeLockController = getQwenQuicknavActiveLockController();
    if (activeLockController && typeof activeLockController.onRouteChange === 'function') {
      activeLockController.onRouteChange(nowMs);
    }

    const routeGateController = getQwenQuicknavRouteGateController();
    if (routeGateController && typeof routeGateController.onRouteChange === 'function') {
      const version = Number(routeGateController.onRouteChange({ href, reason, nowMs }));
      if (Number.isFinite(version)) qwenRouteGateReleasedVersion = version - 1;
    }
    ...
    qwenRouteGatePendingUi = ui;
    if (maybeSampleQwenRouteGate(nowMs)) {
      flushQwenRouteGateReady(ui, nowMs, 'route-change');
    } else {
      ensureQwenRouteGateSampler(ui);
    }
    return true;
  ...
  const provisionalPreviewKeys = new Set();
  let hasProvisionalPreviews = false;
  const roleCache = new Map(); // msgKey -> 'user' | 'assistant'
  const turnIdToPos = new Map(); // turnId -> position in cachedTurns
  let cachedTurns = [];

  function scheduleRefresh(ui, { delay = 80, force = false, soft = false, bypassRouteGate = false } = {}) {
    ...
    function run() {
      ...
      try {
        if (!bypassRouteGate && shouldSuppressRefreshForPendingRoute(ui)) return;
        ...
        if (newCount !== oldCount) {
          setTimeout(() => {
            scheduleRefresh(ui, { force: true, bypassRouteGate });
  ...
  function checkStreamingState(ui, force = false) {
    ...
    const activeLockController = getQwenQuicknavActiveLockController();
    if (activeLockController && typeof activeLockController.onStreaming === 'function') {
      activeLockController.onStreaming(hasStop, now);
    }
    if (lastHasStop === null) {
      lastHasStop = hasStop;
      return hasStop;
  ...
  function init() {
    ...
    const boot = () => {
      ...
      if (document.getElementById('cgpt-compact-nav')) {
        if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 面板已存在，跳过创建');
        return;
      }
      if (qwenQuicknavBooting) {
        if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 正在启动中，跳过重复创建');
        return;
      }

      qwenQuicknavBooting = true;
      try {
        if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 开始创建面板');
        ...
        scheduleRefresh(ui);
        if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 面板创建完成');
      } finally {
        qwenQuicknavBooting = false;
      }
    };
  ...
  function detectUrlChange(payload = null) {
    ...
    const reason = eventPayload && typeof eventPayload.reason === 'string' && eventPayload.reason
      ? eventPayload.reason
      : 'url';
    beginRoutePendingTransition(nextHref, reason);
  }
  ...
  function installRouteWatcher() {
    try {
      if (QUICKNAV_ROUTE_WATCH && typeof QUICKNAV_ROUTE_WATCH.installBridgeFirstWatcher === 'function') {
        const installed = QUICKNAV_ROUTE_WATCH.installBridgeFirstWatcher({
          target: window,
          unsubKey: '__quicknavQwenRouteUnsubV1',
          pollKey: '__quicknavQwenRoutePollV1',
          pollMs: 1200,
    ...
    try {
      if (!window.__quicknavQwenRouteUnsubV1 || typeof window.__quicknavQwenRouteUnsubV1 !== 'function') {
        ...
        if (bridge && typeof bridge.ensureRouteListener === 'function' && typeof bridge.on === 'function') {
          ...
          window.__quicknavQwenRouteUnsubV1 = bridge.on('routeChange', (eventPayload) => detectUrlChange(eventPayload || null));
          return;
        }
      }
    } catch {}
    ...
    try {
      if (window.__quicknavQwenRoutePollV1) return;
      window.__quicknavQwenRoutePollV1 = window.setInterval(() => detectUrlChange({ href: location.href, reason: 'poll' }), 1200);
    } catch {}

    try {
      if (!window.__quicknavQwenChatItemClickHookInstalledV1) {
        window.__quicknavQwenChatItemClickHookInstalledV1 = true;
        document.addEventListener(
          ...
          (event) => {
            ...
            let href = '';
            try {
              href = String(chatItem.href || chatItem.getAttribute?.('href') || '').trim();
            } catch {}
            if (!href) return;
            beginRoutePendingTransition(href, 'chat-item-click');
          },
          true
        );
      }
  ...
  function qsTurns(root = document) {
    ...
    if (TURN_SELECTOR) {
      ...
      if (hit) {
        ...
        if (DEBUG || window.DEBUG_TEMP) {
          console.log(`Qwen QuickNav: 缓存选择器可疑，已重选 ${TURN_SELECTOR} (ratio=${quality.ratio.toFixed(2)}, sample=${quality.sampled})`);
        }
      }
      TURN_SELECTOR = null;
    }
    ...
    if (DEBUG || window.DEBUG_TEMP) {
      console.log('Qwen QuickNav Debug: 检测对话选择器');
    ...
    for (const selector of selectors) {
      ...
      if (quality.suspicious) {
        if (DEBUG || window.DEBUG_TEMP) {
          console.log(`Qwen QuickNav: 跳过可疑选择器 ${selector} (ratio=${quality.ratio.toFixed(2)}, sample=${quality.sampled}, matched=${els.length})`);
        }
        continue;
      }
      TURN_SELECTOR = selector;
      if (DEBUG || window.DEBUG_TEMP) console.log(`Qwen QuickNav: 使用选择器 ${selector}, 找到 ${els.length} 个对话`);
      return Array.from(els);
    }

    if (DEBUG || window.DEBUG_TEMP) {
      console.log('Qwen QuickNav Debug: 所有预设选择器都失效，尝试智能检测');
    ...
    for (const fallbackSelector of fallbackSelectors) {
      ...
      if (candidates.length > 0) {
        if (DEBUG || window.DEBUG_TEMP) console.log(`Qwen QuickNav: Fallback选择器 ${fallbackSelector} 找到 ${candidates.length} 个候选对话`);
        return candidates;
      }
    }

    if (DEBUG) console.log('Qwen QuickNav: 所有检测方法均失效');
    return [];
  }

  function getTextPreview(el) {
    if (!el) return { text: '', provisional: true };
    // 注意：innerText 会触发同步样式/布局计算（长对话非常慢），尽量只用 textContent
    const text = (el.textContent || '').replace(/\s+/g, ' ').trim();
    if (!text) return { text: '...', provisional: true };
    // 让 CSS 负责根据宽度省略，JS 只做上限裁剪以防极端超长文本
    const HARD_CAP = 600;
    return { text: text.length > HARD_CAP ? text.slice(0, HARD_CAP) : text, provisional: false };
  }
  ...
  function getPreviewTrackKey(el, msgKey) {
    if (msgKey) return `k:${msgKey}`;
  ...
  function buildIndex(turnsOverride) {
    ...
    if (!turns.length) {
      provisionalPreviewKeys.clear();
      hasProvisionalPreviews = false;
      if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 没有找到任何对话元素');
      return [];
    }

    if (DEBUG) console.log(`Qwen QuickNav: 开始分析 ${turns.length} 个对话元素`);

    let u = 0, a = 0;
    const list = [];
    const nextProvisionalPreviewKeys = new Set();
    for (let i = 0; i < turns.length; i++) {
      ...
      const previewTrackKey = getPreviewTrackKey(el, msgKey);
      const shouldRetryProvisional = !!(previewTrackKey && provisionalPreviewKeys.has(previewTrackKey));
      let role = msgKey ? (roleCache.get(msgKey) || '') : '';
      ...
      if (!isUser && !isAssistant) {
        ...
        if (role && msgKey) roleCache.set(msgKey, role);
      }

      if (DEBUG && i < 3) {
        console.log(`Qwen QuickNav Debug - 元素 ${i}:`, {
          element: el,
          testId: attrTestId,
          isUser,
          isAssistant,
      ...
      if (!isUser && !isAssistant) {
        if (DEBUG && i < 5) console.log(`Qwen QuickNav: 元素 ${i} 角色识别失败`);
        continue;
      }

      const shouldRecalcPreview = i >= turns.length - TAIL_RECALC_TURNS;
      let preview = msgKey ? (previewCache.get(msgKey) || '') : '';
      let previewIsProvisional = false;
      if (!preview || shouldRecalcPreview || shouldRetryProvisional) {
        let block = null;
        if (isUser) {
        ...
        if (!block) block = el;
        const previewResult = getTextPreview(block);
        preview = previewResult.text;
        previewIsProvisional = !!previewResult.provisional;
        if (msgKey) {
          if (preview && !previewIsProvisional) previewCache.set(msgKey, preview);
          else previewCache.delete(msgKey);
        }
      }
      if (!preview) {
        if (DEBUG && i < 5) console.log(`Qwen QuickNav: 元素 ${i} 无法提取预览文本`);
        continue;
      }

      if (previewIsProvisional && previewTrackKey) {
        nextProvisionalPreviewKeys.add(previewTrackKey);
      }
    ...
    provisionalPreviewKeys.clear();
    nextProvisionalPreviewKeys.forEach((key) => {
      provisionalPreviewKeys.add(key);
    });
    hasProvisionalPreviews = provisionalPreviewKeys.size > 0;

    if (DEBUG) console.log(`Qwen QuickNav: 成功识别 ${list.length} 个对话 (用户: ${u}, 助手: ${a})`);
    return list;
  }
...
`;
      ...
      if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 已创建样式');
    } else {
      if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 样式已存在，跳过创建');
    }

    // 启动前清理多余面板（保险丝）
    const existingPanels = document.querySelectorAll('#cgpt-compact-nav');
    if (existingPanels.length > 0) {
      if (DEBUG || window.DEBUG_TEMP) console.log(`Qwen QuickNav: 发现 ${existingPanels.length} 个已存在的面板，清理中...`);
      existingPanels.forEach((panel, index) => {
        if (index > 0) { // 保留第一个，删除其他
          panel.remove();
          if (DEBUG || window.DEBUG_TEMP) console.log(`Qwen QuickNav: 已删除重复面板 ${index}`);
        }
      });
      // 如果已经有面板存在，直接返回现有的
      if (existingPanels.length > 0) {
        ...
        if (existingNav._ui) {
          if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 返回已存在的面板');
    ...
    try {
      ...
      if (DEBUG || window.DEBUG_TEMP) console.error('Qwen QuickNav: 布局管理器初始化失败', err);
    }
    enableDrag(nav, {
  ...
  function createLayoutManager(nav) {
    ...
    function scheduleEvaluation(reason) {
      ...
      state.rafId = requestAnimationFrame(() => {
        state.rafId = 0;
        state.pendingEval = false;
        try { evaluateNow(reason); } catch (err) { if (DEBUG || window.DEBUG_TEMP) console.error('Qwen QuickNav layout evaluate error:', err); }
      });
    }
  ...
  function renderList(ui) {
    ...
    if (!next.length) {
      ...
      queueScrollbarState();
      syncLockedTurnPresence(Date.now());
      return;
    }
    list.innerHTML = '';
    ...
    if (!list._eventBound) {
      list.addEventListener('click', (e) => {
        ...
        const targetId = String(item.dataset.id || '');
        if (!targetId) return;
        const activeLockController = getQwenQuicknavActiveLockController();
        if (activeLockController && typeof activeLockController.lock === 'function') {
          const nowMs = Date.now();
          const hasStop = !!checkStreamingState(ui, true);
          if (hasStop) {
            if (typeof activeLockController.onStreaming === 'function') {
              activeLockController.onStreaming(true, nowMs);
            }
            activeLockController.lock(targetId, nowMs);
          }
        }
        const el = document.getElementById(targetId);
        if (el) {
          setActiveTurn(targetId);
          scrollToTurn(el);
        }
      });
  ...
  function refreshIndex(ui, { soft = false, force = false } = {}) {
    ...
    if (soft && !force && unchanged && cacheIndex.length && turnCount > 0 && !hasProvisionalPreviews) return false;

    const base = buildIndex(turns);
    const next = composeWithPins(base);
    if (DEBUG) console.log('Qwen QuickNav: turns', next.length, '(含📌)');
  ...
  function wirePanel(ui) {
    ...
    if (refreshBtn) {
      refreshBtn.addEventListener('click', (e) => {
        if (e.shiftKey) {
          // Shift+点击 = 强制重新扫描
          if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 强制重新扫描 (清除缓存选择器)');
      ...
      refreshBtn.addEventListener('contextmenu', (e) => {
        e.preventDefault();
        if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 右键强制重新扫描');
        TURN_SELECTOR = null;
        const originalBg = refreshBtn.style.background;
        const originalColor = refreshBtn.style.color;
        refreshBtn.style.background = 'var(--cgpt-nav-accent-subtle)';
    ...
    // 键盘事件只绑定一次：避免重复绑定
    if (!isCompatBoolFlagSet('__qwenQuicknavKeysBound', '__cgptKeysBound')) {
      const onKeydown = (e) => {
        const t = e.target;
      ...
      setCompatBoolFlag('__qwenQuicknavKeysBound', '__cgptKeysBound', true);
      if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 已绑定键盘事件');
    } else {
      if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 键盘事件已存在，跳过绑定');
    }
  }
  ...
  function getTurnsContainer() {
    ...
    if (!nodes.length) {
      ...
      if (DEBUG && potentialContainers.length > 1) {
        console.log('Qwen QuickNav: 没有找到对话，使用备用容器:', potentialContainers[0]);
      }
    ...
    while (a) {
      if (nodes.every(n => a.contains(n))) {
        if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 对话容器:', a);
        return a;
      }
      a = a.parentElement;
    }
  ...
  function healObserverIfNeeded(ui, reason = '') {
    ...
    if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 观察目标已断开，重绑观察器', reason || '');
    observeChat(ui);
    ...
    const mo = new MutationObserver((muts) => {
      ...
      for (const mut of muts) {
        ...
        if (
          ...
          t.closest('.qwen-chat-message') ||
          t.closest('.qwen-chat-message-user') ||
          t.closest('.qwen-chat-message-assistant') ||
          t.closest('#chat-messages-scroll-container') ||
          t.closest('.text-message[data-author]') ||
          t.closest('.user-message-content') ||
          t.closest('.response-message-content') ||
    ...
    try {
      if (AISHORTCUTS_SCROLLLOCK_BRIDGE && typeof AISHORTCUTS_SCROLLLOCK_BRIDGE.bindReadyHandshake === 'function') {
        const installed = AISHORTCUTS_SCROLLLOCK_BRIDGE.bindReadyHandshake({
          target: window,
          boundKey: '__quicknavScrollGuardHandshakeBound',
  ...
  function bindScrollLockUserIntents() {
    if (isCompatBoolFlagSet('__qwenQuicknavScrollLockUserIntentsBound', '__cgptScrollLockUserIntentsBound')) return;
    setCompatBoolFlag('__qwenQuicknavScrollLockUserIntentsBound', '__cgptScrollLockUserIntentsBound', true);
  ...
  function installScrollGuards() {
    if (isCompatBoolFlagSet('__qwenQuicknavScrollGuardsInstalled', '__cgptScrollGuardsInstalled')) return;
    setCompatBoolFlag('__qwenQuicknavScrollGuardsInstalled', '__cgptScrollGuardsInstalled', true);
  ...
  function initScrollLock(ui) {
    ...
    if (!isCompatBoolFlagSet('__qwenQuicknavScrollLockBound', '__cgptScrollLockBound')) {
      window.addEventListener('resize', () => { if (scrollLockEnabled) ensureScrollLockBindings(); }, { passive: true });
      setCompatBoolFlag('__qwenQuicknavScrollLockBound', '__cgptScrollLockBound', true);
    }
  ...
  function mutationTouchesConversation(node) {
    ...
    if (node.matches('.qwen-chat-message, .qwen-chat-message-user, .qwen-chat-message-assistant, #chat-messages-scroll-container, .text-message[data-author], .user-message-content, .response-message-content')) return true;
    if (node.matches('.markdown, .prose, article')) return true;
    if (node.querySelector?.('[data-message-author-role], [data-message-id], .qwen-chat-message, .qwen-chat-message-user, .qwen-chat-message-assistant, .text-message[data-author], .user-message-content, .response-message-content, .markdown, .prose')) return true;
    return false;
  }
  ...
  function bindAltPin(ui) {
    if (isCompatBoolFlagSet('__qwenQuicknavPinBound', '__cgptPinBound')) return;
    ...
    setCompatBoolFlag('__qwenQuicknavPinBound', '__cgptPinBound', true);
  }
  ...
  function watchSendEvents(ui) {
    if (window.__qwenQuicknavSendEventsBoundV1) return;
    window.__qwenQuicknavSendEventsBoundV1 = true;
    ...
    const hasQwenComposerMarker = (scope) => {
      if (!(scope instanceof Element)) return false;
      try {
        if (scope.matches('#chat-message-input, #chat-input')) return true;
      } catch {}
      try {
        if (scope.closest('#chat-message-input, #chat-input')) return true;
      } catch {}
      try {
        const hasInput = !!scope.querySelector?.(
          'textarea#chat-input, textarea[name="chat-input"], [contenteditable="true"][role="textbox"], [contenteditable="plaintext-only"][role="textbox"]'
        );
        const hasSendControl = !!scope.querySelector?.('button[type="submit"], button.omni-button-content-btn');
        if (hasInput && hasSendControl) return true;
      } catch {}
      return false;
    };

    const getQwenComposerScopeFrom = (target) => {
      const el = asElement(target);
      if (!el) return null;

      try {
        const byContainer = el.closest('#chat-message-input, #chat-input');
        if (byContainer) return byContainer;
      } catch {}

      try {
        const byForm = el.closest('form');
        if (byForm && hasQwenComposerMarker(byForm)) return byForm;
      } catch {}

      try {
        const byChatInputLike = el.closest('[id*="chat-input"], [class*="chat-input"], [data-testid*="chat-input"]');
        if (byChatInputLike && hasQwenComposerMarker(byChatInputLike)) return byChatInputLike;
      } catch {}
    ...
    const isStopLikeControl = (el) => {
      ...
      try {
        ...
        const text = String(el.textContent || '').slice(0, 160);
    ...
    const isComposerForm = (form) => {
      ...
      return hasQwenComposerMarker(form);
    };

    const isSendActionTarget = (target) => {
      const el = asElement(target);
      if (!el) return false;
      const button = el.closest('button[type="submit"], button.omni-button-content-btn');
      if (!button) return false;
      if (!getQwenComposerScopeFrom(button)) return false;
      if (isStopLikeControl(button)) return false;
      return true;
    };

    const isQwenPromptTarget = (target) => {
      const el = asElement(target);
      if (!el) return false;

      try {
        if (el.closest('textarea#chat-input, textarea[name="chat-input"], #chat-message-input textarea')) return true;
      } catch {}

      try {
        const textbox = el.closest('[contenteditable="true"][role="textbox"], [contenteditable="plaintext-only"][role="textbox"]');
        if (textbox && getQwenComposerScopeFrom(textbox)) return true;
      } catch {}
    ...
    document.addEventListener('click', (e) => {
      ...
      if (!currentUi) return;
      if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 检测到发送按钮点击，启动突发刷新');
      armScrollLockGuard(2200);
    ...
    document.addEventListener('submit', (e) => {
      ...
      const scope = getQwenComposerScopeFrom(form);
      const stopLike = queryStopLikeControl(scope || form) || queryStopLikeControl(document);
      if (stopLike) return;
      const currentUi = getUi();
      if (!currentUi) return;
      if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 检测到表单提交，启动突发刷新');
      armScrollLockGuard(2200);
      startBurstRefresh(currentUi);
    }, true);
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (!isQwenPromptTarget(e.target)) return;
      const scope = getQwenComposerScopeFrom(e.target) || document.querySelector('#chat-message-input, #chat-input') || document;
      const stopLike = queryStopLikeControl(scope) || queryStopLikeControl(document);
      if (stopLike) return;
      const currentUi = getUi();
      if (!currentUi) return;
      if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 检测到快捷键发送，启动突发刷新');
      armScrollLockGuard(2200);
      startBurstRefresh(currentUi);
    }, true);
    ...
    document.addEventListener('visibilitychange', () => {
      if (!document.hidden) {
        const currentUi = getUi();
        if (!currentUi) return;
        if (DEBUG || window.DEBUG_TEMP) console.log('Qwen QuickNav: 页面重新可见，强制刷新');
  ...
  function updateActiveFromAnchor() {
    const nowMs = Date.now();
    const activeLockController = getQwenQuicknavActiveLockController();
    if (activeLockController && typeof activeLockController.shouldAllowAutoActiveUpdate === 'function') {
      if (!activeLockController.shouldAllowAutoActiveUpdate(nowMs)) return;
    }
    ...
    if (!activeEl) {
      const sinceScroll = nowMs - (lastScrollTs || 0);
