...
`;
  ...
  async function toggleThinkingFast() {
    ...
    if (!openTarget || !dispatchUserClick(openTarget)) {
    ...
    const fallbackLabel = getOptionText(option) || modeLabel(want) || '目标模式';
    showToast(`Qwen: 模式 → ${after || fallbackLabel}`);
  }
