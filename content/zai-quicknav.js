(function () {
  ...
  window.chatGptNavDebug = {
    ...
    checkOverlap: () => {
      ...
      console.log(`键盘事件已绑定: ${readRuntimeFlag('__quicknavZaiKeysBoundV1', '__cgptKeysBound')}`);
      ...
      return { panels: panels.length, styles: styles.length, keysBound: readRuntimeFlag('__quicknavZaiKeysBoundV1', '__cgptKeysBound'), booting: __cgptBooting };
    },
    testObserver: () => {
  ...
  function detectUrlChange(payload = null) {
    ...
    if (nextHref !== currentUrl) {
      ...
      writeRuntimeFlag('__quicknavZaiKeysBoundV1', '__cgptKeysBound', false);
      lastTurnCount = 0;
  ...
  function installRouteWatcher() {
    try {
      if (QUICKNAV_ROUTE_WATCH && typeof QUICKNAV_ROUTE_WATCH.installBridgeFirstWatcher === 'function') {
        const installed = QUICKNAV_ROUTE_WATCH.installBridgeFirstWatcher({
          target: window,
          unsubKey: '__quicknavZaiRouteUnsubV1',
          pollKey: '__quicknavZaiRoutePollV1',
          pollMs: 1200,
    ...
    try {
      if (window.__quicknavZaiManualRouteWatcherV1) return;
      window.__quicknavZaiManualRouteWatcherV1 = true;
    } catch {}
...
`;
  ...
  function wirePanel(ui) {
    ...
    // 键盘事件只绑定一次：避免重复绑定
    if (!readRuntimeFlag('__quicknavZaiKeysBoundV1', '__cgptKeysBound')) {
      const onKeydown = (e) => {
        const t = e.target;
      ...
      writeRuntimeFlag('__quicknavZaiKeysBoundV1', '__cgptKeysBound', true);
  ...
  function bindScrollLockUserIntents() {
    if (readRuntimeFlag('__quicknavZaiScrollLockUserIntentsBound', '__cgptScrollLockUserIntentsBound')) return;
    writeRuntimeFlag('__quicknavZaiScrollLockUserIntentsBound', '__cgptScrollLockUserIntentsBound', true);
  ...
  function installScrollGuards() {
    if (readRuntimeFlag('__quicknavZaiScrollGuardsInstalled', '__cgptScrollGuardsInstalled')) return;
    writeRuntimeFlag('__quicknavZaiScrollGuardsInstalled', '__cgptScrollGuardsInstalled', true);
  ...
  function initScrollLock(ui) {
    ...
    if (!readRuntimeFlag('__quicknavZaiScrollLockBound', '__cgptScrollLockBound')) {
      window.addEventListener('resize', () => { if (scrollLockEnabled) ensureScrollLockBindings(); }, { passive: true });
      writeRuntimeFlag('__quicknavZaiScrollLockBound', '__cgptScrollLockBound', true);
    }
  ...
  function bindAltPin(ui) {
    if (readRuntimeFlag('__quicknavZaiPinBound', '__cgptPinBound')) return;
    ...
    writeRuntimeFlag('__quicknavZaiPinBound', '__cgptPinBound', true);
  }
