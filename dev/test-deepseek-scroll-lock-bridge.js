...
const FILE = path.join(ROOT, 'content', 'deepseek-quicknav.js');
...
function testHandshakeAndRouteWatcher(source) {
  ...
  expectRegex(
    source,
    /installRouteWatcher\(\)\s*\{[\s\S]*?bridge\.on\(\s*'routeChange'\s*,\s*\(eventPayload\)\s*=>\s*detectUrlChange\(eventPayload\s*\|\|\s*null\)\s*\)[\s\S]*?window\.setInterval\(\(\)\s*=>\s*detectUrlChange\(\s*\{\s*href:\s*location\.href,\s*reason:\s*'poll'\s*\}\s*\),\s*1200\);[\s\S]*?\}/,
    'route watcher should prefer bridge routeChange and keep polling fallback'
  );

  assert.ok(!/history\.pushState\s*=\s*function\s*\(/.test(source), 'deepseek quicknav should avoid per-script history.pushState patching');
  assert.ok(!/history\.replaceState\s*=\s*function\s*\(/.test(source), 'deepseek quicknav should avoid per-script history.replaceState patching');
}

function testRuntimeCompatibilityFlags(source) {
  expectRegex(
    source,
    /const\s+DEEPSEEK_KEYS_BOUND_KEY\s*=\s*'__deepseekQuicknavKeysBound';[\s\S]*?const\s+DEEPSEEK_KEYS_BOUND_LEGACY_KEY\s*=\s*'__cgptKeysBound';/,
    'should define deepseek + legacy key-binding flags'
  );

  expectRegex(
    source,
    /function\s+isDeepseekKeysBound\(\)\s*\{[\s\S]*?isCompatBoolFlagSet\(\s*DEEPSEEK_KEYS_BOUND_KEY,\s*DEEPSEEK_KEYS_BOUND_LEGACY_KEY\s*\)/,
    'should expose typed key-binding flag readers'
  );

  expectRegex(
    source,
    /window\.deepseekQuicknavDebug\s*=\s*deepseekQuicknavDebugApi;[\s\S]*?window\.deepseekNavDebug\s*=\s*deepseekQuicknavDebugApi;[\s\S]*?window\.chatGptNavDebug\s*=\s*deepseekQuicknavDebugApi;/,
    'should expose deepseek debug aliases while preserving legacy alias'
  );
}
...
function main() {
  ...
  testRuntimeCompatibilityFlags(source);
  console.log('PASS dev/test-deepseek-scroll-lock-bridge.js');
}
