...
function testHandshakeAndRouteWatcher(source) {
  ...
  expectRegex(
    source,
    /installRouteWatcher\(\)\s*\{[\s\S]*?bridge\.on\(\s*'routeChange'\s*,\s*\(eventPayload\)\s*=>\s*detectUrlChange\(eventPayload\s*\|\|\s*null\)\s*\)[\s\S]*?window\.setInterval\([\s\S]*?detectUrlChange\(\{\s*href:\s*location\.href,\s*reason:\s*'poll'\s*\}\)[\s\S]*?1200[\s\S]*?\);[\s\S]*?\}/,
    'route watcher should prefer bridge routeChange and keep polling fallback'
  );

  assert.ok(!/history\.pushState\s*=\s*function\s*\(/.test(source), 'genspark quicknav should avoid per-script history.pushState patching');
  assert.ok(!/history\.replaceState\s*=\s*function\s*\(/.test(source), 'genspark quicknav should avoid per-script history.replaceState patching');
}
...
function main() {
  ...
  console.log('PASS dev/test-genspark-scroll-lock-bridge.js');
}
