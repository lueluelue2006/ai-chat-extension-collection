(() => {
  ...
  function handleAdminMessage(context) {
    const {
      msg,
      sender,
      sendResponse,
    ...
    if (msg.type === "AISHORTCUTS_DIAG_GET_DUMP") {
      ...
      try {
        const tail = Number.isFinite(msg?.tail) ? Math.max(0, Math.floor(msg.tail)) : void 0;
        const dump = ns.diag && typeof ns.diag.dump === "function" ? ns.diag.dump(typeof tail === "number" ? { tail } : {}) : { max: 0, size: 0, droppedDuplicateMsgCount: 0, events: [] };
    ...
    if (msg.type === "AISHORTCUTS_NOTIFY") {
      ...
      ns.chrome.notificationsCreate(notifyId, {
        type: "basic",
        iconUrl: chrome.runtime.getURL("icons/icon128.png"),
        title: title.slice(0, 64),
        message: message.slice(0, 256),
        priority: 1
      }).then(() => sendResponse({ ok: true })).catch((error) => respondError(sendResponse, error));
      return true;
    }
    if (msg.type === "AISHORTCUTS_FACTORY_RESET") {
      ...
      })().catch((error) => respondError(sendResponse, error)).finally(() => {
    ...
    if (msg.type === "AISHORTCUTS_REINJECT_NOW") {
      if (!requireAllowedSender(sendResponse, sender)) return true;
      ns.storage.getSettings().then((settings) => {
