(() => {
  ...
  let DEFAULT_SETTINGS = { enabled: true, metaKeyMode: "auto", localeMode: "auto", sites: {}, scrollLockDefaults: {}, siteModules: {} };
  let MAIN_GUARD_FILE = "content/scroll-guard-main.js";
  let CONTENT_SCRIPT_DEFS = [];
  let LEGACY_CONTENT_SCRIPT_IDS = [];
  let KNOWN_SITE_IDS = /* @__PURE__ */ new Set();
  let KNOWN_SITE_MODULE_KEYS = /* @__PURE__ */ new Map();
  let MODULE_ID_ALIASES = {};
  let MODULE_ALIAS_TARGETS = /* @__PURE__ */ new Map();
  let settingsMutationChain = Promise.resolve();
  let hardCutSchemaInitialized = false;
  let hardCutSchemaInitPromise = null;
  let hardCutSchemaLastReport = null;
  ...
  function buildStorageKeyList(input) {
  ...
  async function listPresentStorageKeys(areaName, keys) {
  ...
  async function purgeLegacyKeysInArea(areaName) {
  ...
  async function purgeLegacyProductStorageKeys() {
    const removedByArea = { local: [], sync: [], session: [] };
  ...
  function buildHardCutReportBase(options = {}) {
    return {
      force: !!options.force,
  ...
  function buildHardCutFallbackReport(options, markerWriteError) {
  ...
  async function ensureHardCutStorageSchema(options = {}) {
    ...
    const run = async () => {
      ...
      const report = Object.assign(buildHardCutReportBase({ force }), {
        ...
        removedByArea: purgeReport && typeof purgeReport === "object" ? purgeReport.removedByArea || { local: [], sync: [], session: [] } : { local: [], sync: [], session: [] }
      });
    ...
    hardCutSchemaInitPromise = run().catch((error) => {
      const fallback = buildHardCutFallbackReport({ force }, error);
      hardCutSchemaLastReport = fallback;
      return fallback;
    }).finally(() => {
  ...
  function getLegacyModuleIdsForCanonical(canonicalId) {
    const key = String(canonicalId || "").trim();
    if (!key) return [];
    const list = MODULE_ALIAS_TARGETS.get(key);
    return Array.isArray(list) ? list : [];
  }
  function normalizeMetaKeyMode(input, fallback = "auto") {
    const value = String(input || "").trim().toLowerCase();
    if (value === "auto" || value === "has_meta" || value === "no_meta") return value;
    return fallback;
  }
  function normalizeLocaleMode(input, fallback = "auto") {
    const value = String(input || "").trim().toLowerCase();
  ...
  function rebuildKnownKeys() {
    KNOWN_SITE_IDS = /* @__PURE__ */ new Set();
    KNOWN_SITE_MODULE_KEYS = /* @__PURE__ */ new Map();
    try {
    ...
    try {
      const siteModules = DEFAULT_SETTINGS?.siteModules && typeof DEFAULT_SETTINGS.siteModules === "object" ? DEFAULT_SETTINGS.siteModules : {};
  ...
  function initConfig(config) {
    ...
    MODULE_ID_ALIASES = {};
    MODULE_ALIAS_TARGETS = /* @__PURE__ */ new Map();
    try {
