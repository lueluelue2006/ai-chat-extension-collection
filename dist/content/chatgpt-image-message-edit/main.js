(() => {
    ...
    try {
      ...
      btn.addEventListener(
        ...
        (e) => {
          e.preventDefault();
          e.stopPropagation();
          // ChatGPT's virtualized list may recycle DOM nodes; resolve the current turn at click-time.
          const currentTurn =
            e.currentTarget instanceof Element ? e.currentTarget.closest(ANY_TURN_SELECTOR) : null;
          void enterEditMode(currentTurn || turnEl);
        },
        true
      );
  ...
  function installPayloadRewriter() {
    ...
    if (registerConsumer) {
      ...
      state.hubUnsub = registerConsumer('chatgpt-image-message-edit', {
        priority: 100,
        onConversationStart: () => {
