(function () {
  ...
  const GEMINI_APP_NAV_DEBUG_KEY = 'geminiNavDebug';
  const GEMINI_APP_NAV_DEBUG_LEGACY_KEY = 'chatGptNavDebug';
  const GEMINI_APP_KEYS_BOUND_KEY = '__quicknavGeminiAppKeysBoundV1';
  const GEMINI_APP_KEYS_BOUND_LEGACY_KEY = '__cgptKeysBound';
  const GEMINI_APP_SCROLL_LOCK_USER_INTENTS_BOUND_KEY = '__quicknavGeminiAppScrollLockUserIntentsBoundV1';
  const GEMINI_APP_SCROLL_LOCK_USER_INTENTS_BOUND_LEGACY_KEY = '__cgptScrollLockUserIntentsBound';
  const GEMINI_APP_SCROLL_GUARDS_INSTALLED_KEY = '__quicknavGeminiAppScrollGuardsInstalledV1';
  const GEMINI_APP_SCROLL_GUARDS_INSTALLED_LEGACY_KEY = '__cgptScrollGuardsInstalled';
  const GEMINI_APP_SCROLL_LOCK_BOUND_KEY = '__quicknavGeminiAppScrollLockBoundV1';
  const GEMINI_APP_SCROLL_LOCK_BOUND_LEGACY_KEY = '__cgptScrollLockBound';
  const GEMINI_APP_PIN_BOUND_KEY = '__quicknavGeminiAppPinBoundV1';
  ...
  function isGeminiAppKeysBound() {
    return readRuntimeFlag(GEMINI_APP_KEYS_BOUND_KEY, GEMINI_APP_KEYS_BOUND_LEGACY_KEY);
  }

  function setGeminiAppKeysBound(value) {
    return writeRuntimeFlag(GEMINI_APP_KEYS_BOUND_KEY, GEMINI_APP_KEYS_BOUND_LEGACY_KEY, value);
  }

  function isGeminiAppScrollLockUserIntentsBound() {
    return readRuntimeFlag(
      GEMINI_APP_SCROLL_LOCK_USER_INTENTS_BOUND_KEY,
      GEMINI_APP_SCROLL_LOCK_USER_INTENTS_BOUND_LEGACY_KEY
    );
  }

  function setGeminiAppScrollLockUserIntentsBound(value) {
    return writeRuntimeFlag(
      GEMINI_APP_SCROLL_LOCK_USER_INTENTS_BOUND_KEY,
      GEMINI_APP_SCROLL_LOCK_USER_INTENTS_BOUND_LEGACY_KEY,
      value
    );
  ...
  function isGeminiAppScrollGuardsInstalled() {
    return readRuntimeFlag(GEMINI_APP_SCROLL_GUARDS_INSTALLED_KEY, GEMINI_APP_SCROLL_GUARDS_INSTALLED_LEGACY_KEY);
  }

  function setGeminiAppScrollGuardsInstalled(value) {
    return writeRuntimeFlag(GEMINI_APP_SCROLL_GUARDS_INSTALLED_KEY, GEMINI_APP_SCROLL_GUARDS_INSTALLED_LEGACY_KEY, value);
  }

  function isGeminiAppScrollLockBound() {
    return readRuntimeFlag(GEMINI_APP_SCROLL_LOCK_BOUND_KEY, GEMINI_APP_SCROLL_LOCK_BOUND_LEGACY_KEY);
  }

  function setGeminiAppScrollLockBound(value) {
    return writeRuntimeFlag(GEMINI_APP_SCROLL_LOCK_BOUND_KEY, GEMINI_APP_SCROLL_LOCK_BOUND_LEGACY_KEY, value);
  }

  function isGeminiAppPinBound() {
    return readRuntimeFlag(GEMINI_APP_PIN_BOUND_KEY, GEMINI_APP_PIN_BOUND_LEGACY_KEY);
  }

  function setGeminiAppPinBound(value) {
    return writeRuntimeFlag(GEMINI_APP_PIN_BOUND_KEY, GEMINI_APP_PIN_BOUND_LEGACY_KEY, value);
  }

  function installGeminiAppNavDebugApi(debugApi) {
    try { window[GEMINI_APP_NAV_DEBUG_KEY] = debugApi; } catch {}
    try { window[GEMINI_APP_NAV_DEBUG_LEGACY_KEY] = debugApi; } catch {}
  }

  // Gemini App 可能启用 Trusted Types：innerHTML 需要 TrustedHTML
  const TT_POLICY = (() => {
    try {
  ...
  // 全局调试函数，用户可在控制台调用
  const geminiAppQuicknavDebugApi = {
    forceRefresh: () => {
    ...
    checkOverlap: () => {
      ...
      console.log(`键盘事件已绑定: ${isGeminiAppKeysBound()}`);
      console.log(`正在启动中: ${geminiAppQuicknavBooting}`);
      if (panels.length > 1) {
      ...
      return {
        ...
        keysBound: isGeminiAppKeysBound(),
        booting: geminiAppQuicknavBooting
      };
    },
    testObserver: () => {
  ...
  installGeminiAppNavDebugApi(geminiAppQuicknavDebugApi);
  ...
  let manualSelectionHoldUntil = 0;
  const MANUAL_SELECTION_HOLD_MS_NORMAL = 900;
  const MANUAL_SELECTION_HOLD_MS_STREAMING = 1800;
  let geminiAppQuicknavBooting = false;
  ...
  function getManualSelectionHoldMs() {
    try {
      return checkStreamingState(null, true) ? MANUAL_SELECTION_HOLD_MS_STREAMING : MANUAL_SELECTION_HOLD_MS_NORMAL;
    } catch {
      return MANUAL_SELECTION_HOLD_MS_NORMAL;
    }
  }

  function armManualSelectionHold(ms = 0) {
    const n = Number(ms);
    const holdMs = Number.isFinite(n) && n > 0 ? n : getManualSelectionHoldMs();
  ...
  function init() {
    ...
    const boot = () => {
      ...
      try {
        ...
        geminiAppQuicknavBooting = false;
      }
    };
    const tryBoot = () => {
    ...
    // Gemini App 的关键 DOM 多在 open shadow roots 内，单靠监听 document.body 可能捕捉不到。
  ...
  function detectUrlChange(payload = null) {
    ...
    if (nextHref !== currentUrl) {
      const reason = eventPayload && typeof eventPayload.reason === 'string' && eventPayload.reason ? eventPayload.reason : 'url';
      if (DEBUG || window.DEBUG_TEMP) console.log('ChatGPT Navigation: URL变化，清理旧实例', currentUrl, '->', nextHref, `[${reason}]`);
      currentUrl = nextHref;
  ...
  function installRouteWatcher() {
    try {
      if (QUICKNAV_ROUTE_WATCH && typeof QUICKNAV_ROUTE_WATCH.installBridgeFirstWatcher === 'function') {
        const installed = QUICKNAV_ROUTE_WATCH.installBridgeFirstWatcher({
          target: window,
          unsubKey: GEMINI_APP_ROUTE_UNSUB_KEY,
          pollKey: GEMINI_APP_ROUTE_POLL_KEY,
          pollMs: 1200,
    ...
    try {
      if (!window[GEMINI_APP_ROUTE_UNSUB_KEY] || typeof window[GEMINI_APP_ROUTE_UNSUB_KEY] !== 'function') {
        ...
        if (bridge && typeof bridge.ensureRouteListener === 'function' && typeof bridge.on === 'function') {
          ...
          window[GEMINI_APP_ROUTE_UNSUB_KEY] = bridge.on('routeChange', (eventPayload) => detectUrlChange(eventPayload || null));
          return;
        }
      }
    } catch {}
    ...
    try {
      if (window[GEMINI_APP_ROUTE_POLL_KEY]) return;
      window[GEMINI_APP_ROUTE_POLL_KEY] = window.setInterval(
  ...
  function normalizeGeminiPreview(text, role = 'user') {
    let out = String(text || '').replace(/\s+/g, ' ').trim();
    if (!out) return '...';

    if (role === 'user') {
      out = out.replace(/^(?:you\s+said|you\s+asked|you|你说|你问|您说|你输入了|您输入了)\s*[:：\-–—]?\s*/i, '');
    } else if (role === 'assistant') {
      out = out.replace(/^(?:gemini\s+said|gemini|助手说|模型说|回答|回复)\s*[:：\-–—]?\s*/i, '');
    }
  ...
  function extractGeminiTurnParts(turnEl) {
    ...
    const qPreviewDoc = uq?.querySelector('.query-text .query-text-line, .query-text-line') || qDoc || null;
    const aDoc = mr?.querySelector('message-content') || mr || null;
    const questionText = normalizeGeminiPreview(qPreviewDoc ? getTextPreview(qPreviewDoc) : '', 'user');
    const answerText = normalizeGeminiPreview(aDoc ? getTextPreview(aDoc) : '', 'assistant');
  ...
  function buildIndex(turnsOverride) {
    ...
    for (let i = 0; i < turns.length; i++) {
      ...
      // 兼容旧缓存：历史 data-gq-*preview 可能包含 “You said / Gemini said” 前缀，统一在构建阶段做一次净化
      const normalizedQPreview = normalizeGeminiPreview(qPreview, 'user');
      if (normalizedQPreview !== qPreview) {
        qPreview = normalizedQPreview;
        turn.setAttribute('data-gq-qpreview', qPreview);
      }
      const normalizedAPreview = normalizeGeminiPreview(aPreview, 'assistant');
      if (normalizedAPreview !== aPreview) {
        aPreview = normalizedAPreview;
        turn.setAttribute('data-gq-apreview', aPreview);
      }
...
`;
  ...
  function findLeftSidebarElement() {
    // Gemini App: left sidebar lives inside `ucs-nav-panel` (open shadow root).
    try {
      const root = getGeminiRoot();
      if (!root) return null;
  ...
  function findRightPanelElement() {
    // Gemini App: right panel lives inside `ucs-results` -> `ucs-sources-panel` / `.desktop-sources-panel`.
    try {
      const root = getGeminiRoot();
      if (!root) return null;
  ...
  function findGeminiTopHeaderElement() {
    // Gemini App: header bar container is inside `ucs-standalone-app.shadowRoot`.
    try {
      const root = getGeminiRoot();
      if (!root) return null;
  ...
  function renderList(ui) {
    ...
    queueScrollbarState();
    // 重新渲染会丢失 active class：根据 currentActiveId 立即恢复一次，避免流式刷新期间闪烁/跳选
    if (currentActiveId) {
  ...
  function bindScrollLockUserIntents() {
    if (isGeminiAppScrollLockUserIntentsBound()) return;
    setGeminiAppScrollLockUserIntentsBound(true);
  ...
  function installScrollGuards() {
    if (isGeminiAppScrollGuardsInstalled()) return;
    setGeminiAppScrollGuardsInstalled(true);
  ...
  function mutationTouchesConversation(node) {
    ...
    // Gemini App: 主要在 open shadow DOM 内
  ...
  function watchSendEvents(ui) {
    ...
    const isSendButton = (el) => {
      try {
        ...
        // Gemini App uses aria-label="Submit" on the send button.
  ...
  function updateActiveFromAnchor() {
    ...
    if (manualSelectionHoldUntil && Date.now() < manualSelectionHoldUntil) return;
    const y = getAnchorY();

    // Gemini App 的主要内容位于 open shadow DOM 内，document.elementsFromPoint()
