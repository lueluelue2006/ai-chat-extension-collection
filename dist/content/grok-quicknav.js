(function () {
  ...
  const GROK_KEYS_BOUND_KEY = '__quicknavGrokKeysBoundV1';
  const GROK_KEYS_BOUND_LEGACY_KEY = '__cgptKeysBound';
  const GROK_SCROLL_LOCK_USER_INTENTS_BOUND_KEY = '__quicknavGrokScrollLockUserIntentsBound';
  const GROK_SCROLL_LOCK_USER_INTENTS_BOUND_LEGACY_KEY = '__cgptScrollLockUserIntentsBound';
  const GROK_SCROLL_GUARDS_INSTALLED_KEY = '__quicknavGrokScrollGuardsInstalled';
  const GROK_SCROLL_GUARDS_INSTALLED_LEGACY_KEY = '__cgptScrollGuardsInstalled';
  const GROK_SCROLL_LOCK_BOUND_KEY = '__quicknavGrokScrollLockBound';
  const GROK_SCROLL_LOCK_BOUND_LEGACY_KEY = '__cgptScrollLockBound';
  const GROK_PIN_BOUND_KEY = '__quicknavGrokPinBound';
  const GROK_PIN_BOUND_LEGACY_KEY = '__cgptPinBound';
  const GROK_SEND_EVENTS_BOUND_KEY = '__quicknavGrokSendEventsBoundV1';
  ...
  function isGrokKeysBound() {
    return readRuntimeFlag(GROK_KEYS_BOUND_KEY, GROK_KEYS_BOUND_LEGACY_KEY);
  }

  function setGrokKeysBound(value) {
    return writeRuntimeFlag(GROK_KEYS_BOUND_KEY, GROK_KEYS_BOUND_LEGACY_KEY, value);
  }

  function isGrokScrollLockUserIntentsBound() {
    return readRuntimeFlag(GROK_SCROLL_LOCK_USER_INTENTS_BOUND_KEY, GROK_SCROLL_LOCK_USER_INTENTS_BOUND_LEGACY_KEY);
  }

  function setGrokScrollLockUserIntentsBound(value) {
    return writeRuntimeFlag(GROK_SCROLL_LOCK_USER_INTENTS_BOUND_KEY, GROK_SCROLL_LOCK_USER_INTENTS_BOUND_LEGACY_KEY, value);
  }

  function isGrokScrollGuardsInstalled() {
    return readRuntimeFlag(GROK_SCROLL_GUARDS_INSTALLED_KEY, GROK_SCROLL_GUARDS_INSTALLED_LEGACY_KEY);
  }

  function setGrokScrollGuardsInstalled(value) {
    return writeRuntimeFlag(GROK_SCROLL_GUARDS_INSTALLED_KEY, GROK_SCROLL_GUARDS_INSTALLED_LEGACY_KEY, value);
  }

  function isGrokScrollLockBound() {
    return readRuntimeFlag(GROK_SCROLL_LOCK_BOUND_KEY, GROK_SCROLL_LOCK_BOUND_LEGACY_KEY);
  }

  function setGrokScrollLockBound(value) {
    return writeRuntimeFlag(GROK_SCROLL_LOCK_BOUND_KEY, GROK_SCROLL_LOCK_BOUND_LEGACY_KEY, value);
  }

  function isGrokPinBound() {
    return readRuntimeFlag(GROK_PIN_BOUND_KEY, GROK_PIN_BOUND_LEGACY_KEY);
  }

  function setGrokPinBound(value) {
    return writeRuntimeFlag(GROK_PIN_BOUND_KEY, GROK_PIN_BOUND_LEGACY_KEY, value);
  }

  function isGrokSendEventsBound() {
    try {
      return !!window[GROK_SEND_EVENTS_BOUND_KEY];
    } catch {
      return false;
    }
  }

  function setGrokSendEventsBound(value) {
    const normalized = !!value;
    try { window[GROK_SEND_EVENTS_BOUND_KEY] = normalized; } catch {}
    return normalized;
  }
...
`;
  ...
  // 全局调试函数，用户可在控制台调用
  const grokQuicknavDebugApi = {
    forceRefresh: () => {
      console.log('Grok QuickNav: 手动强制刷新');
      TURN_SELECTOR = null;
    ...
    checkOverlap: () => {
      ...
      console.log(`键盘事件已绑定: ${isGrokKeysBound()}`);
      console.log(`正在启动中: ${grokQuicknavBooting}`);
      if (panels.length > 1) {
      ...
      return { panels: panels.length, styles: styles.length, keysBound: isGrokKeysBound(), booting: grokQuicknavBooting };
    },
    testObserver: () => {
      const nav = document.getElementById('cgpt-compact-nav');
  ...
  window.grokQuicknavDebug = grokQuicknavDebugApi;
  window.grokNavDebug = grokQuicknavDebugApi;
  window.chatGptNavDebug = grokQuicknavDebugApi; // backward compatible alias
  ...
  function isGrokConversationPath(pathname = location.pathname) {
    try {
  ...
  function init() {
    ...
    const boot = () => {
      ...
      if (document.getElementById('cgpt-compact-nav')) {
        if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 面板已存在，跳过创建');
        return;
      }
      if (grokQuicknavBooting) {
        if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 正在启动中，跳过重复创建');
        return;
      }

      grokQuicknavBooting = true;
      try {
        if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 开始创建面板');
        ...
        scheduleRefresh(ui);
        if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 面板创建完成');
      } finally {
        grokQuicknavBooting = false;
      }
    };
    if (checkContentLoaded()) boot();
    else {
      // Grok is an SPA: starting on "/" should not keep a global observer alive.
      // Route watcher will trigger `init()` again after navigating to `/c/...`.
      if (!isGrokConversationPath()) return;
  ...
  function detectUrlChange(payload = null) {
    ...
    if (nextHref !== currentUrl) {
      if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: URL变化，清理旧实例', currentUrl, '->', nextHref);
      currentUrl = nextHref;
      ...
      if (oldNav) {
        if (oldNav._ui) {
          ...
          if (oldNav._ui._mo) {
            try {
              ...
              if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 已断开MutationObserver');
            } catch (e) {
              if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 断开MutationObserver失败', e);
            }
          }
        ...
        oldNav.remove();
        if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 已移除旧面板');
      }
      // 重置"正在启动"标志，避免新页面被卡住
      grokQuicknavBooting = false;
      lastTurnCount = 0;
  ...
  (function installRouteWatcher() {
    try {
      if (QUICKNAV_ROUTE_WATCH && typeof QUICKNAV_ROUTE_WATCH.installBridgeFirstWatcher === 'function') {
        const installed = QUICKNAV_ROUTE_WATCH.installBridgeFirstWatcher({
          target: window,
          unsubKey: '__quicknavGrokRouteUnsubV1',
          pollKey: '__quicknavGrokRoutePollV1',
          pollMs: 1200,
    ...
    try {
      if (window.__quicknavGrokRouteUnsubV1 && typeof window.__quicknavGrokRouteUnsubV1 === 'function') return;
      ...
      if (bridge && typeof bridge.ensureRouteListener === 'function' && typeof bridge.on === 'function') {
        ...
        window.__quicknavGrokRouteUnsubV1 = bridge.on('routeChange', (eventPayload) => detectUrlChange(eventPayload || null));
        return;
      }
    } catch {}
    ...
    try {
      if (window.__quicknavGrokRoutePollV1) return;
      window.__quicknavGrokRoutePollV1 = window.setInterval(() => detectUrlChange({ href: location.href, reason: 'poll' }), 1200);
    } catch {}
  })();
  ...
  function buildIndex(turnsOverride) {
    ...
    turnIdToPos.clear();
    if (!turns.length) {
      if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 没有找到任何对话元素');
      return [];
    }

    if (DEBUG) console.log(`Grok QuickNav: 开始分析 ${turns.length} 个对话元素`);
    ...
    for (let i = 0; i < turns.length; i++) {
      ...
      if (DEBUG && i < 3) {
        console.log(`Grok QuickNav Debug - 元素 ${i}:`, {
          element: el,
          testId: attrTestId,
          isUser,
          isAssistant,
      ...
      if (!isUser && !isAssistant) {
        if (DEBUG && i < 5) console.log(`Grok QuickNav: 元素 ${i} 角色识别失败`);
        continue;
      }
      ...
      if (!preview) {
        if (DEBUG && i < 5) console.log(`Grok QuickNav: 元素 ${i} 无法提取预览文本`);
        continue;
      }
    ...
    if (DEBUG) console.log(`Grok QuickNav: 成功识别 ${list.length} 个对话 (用户: ${u}, 助手: ${a})`);
    return list;
  }
...
`;
      ...
      if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 已创建样式');
    } else {
      if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 样式已存在，跳过创建');
    }

    // 启动前清理多余面板（保险丝）
    const existingPanels = document.querySelectorAll('#cgpt-compact-nav');
    if (existingPanels.length > 0) {
      if (DEBUG || window.DEBUG_TEMP) console.log(`Grok QuickNav: 发现 ${existingPanels.length} 个已存在的面板，清理中...`);
      existingPanels.forEach((panel, index) => {
        if (index > 0) { // 保留第一个，删除其他
          panel.remove();
          if (DEBUG || window.DEBUG_TEMP) console.log(`Grok QuickNav: 已删除重复面板 ${index}`);
        }
      });
      // 如果已经有面板存在，直接返回现有的
      if (existingPanels.length > 0) {
        ...
        if (existingNav._ui) {
          if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 返回已存在的面板');
    ...
    try {
      ...
      if (DEBUG || window.DEBUG_TEMP) console.error('Grok QuickNav: 布局管理器初始化失败', err);
    }
    enableDrag(nav, {
  ...
  function createLayoutManager(nav) {
    ...
    function scheduleEvaluation(reason) {
      ...
      state.rafId = requestAnimationFrame(() => {
        state.rafId = 0;
        state.pendingEval = false;
        try { evaluateNow(reason); } catch (err) { if (DEBUG || window.DEBUG_TEMP) console.error('Grok QuickNav layout evaluate error:', err); }
      });
    }
  ...
  function refreshIndex(ui, { soft = false, force = false } = {}) {
    ...
    if (DEBUG) console.log('Grok QuickNav: turns', next.length, '(含📌)');
  ...
  function wirePanel(ui) {
    ...
    if (refreshBtn) {
      refreshBtn.addEventListener('click', (e) => {
        if (e.shiftKey) {
          // Shift+点击 = 强制重新扫描
          if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 强制重新扫描 (清除缓存选择器)');
      ...
      refreshBtn.addEventListener('contextmenu', (e) => {
        e.preventDefault();
        if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 右键强制重新扫描');
        TURN_SELECTOR = null;
        const originalBg = refreshBtn.style.background;
        const originalColor = refreshBtn.style.color;
        refreshBtn.style.background = 'var(--cgpt-nav-accent-subtle)';
    ...
    if (!isGrokKeysBound()) {
      ...
      setGrokKeysBound(true);
      if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 已绑定键盘事件');
    } else {
      if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 键盘事件已存在，跳过绑定');
    }
  }
  ...
  function getTurnsContainer() {
    ...
    if (!nodes.length) {
      ...
      if (DEBUG && potentialContainers.length > 1) {
        console.log('Grok QuickNav: 没有找到对话，使用备用容器:', potentialContainers[0]);
      }
    ...
    while (a) {
      if (nodes.every(n => a.contains(n))) {
        if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 对话容器:', a);
        return a;
      }
      a = a.parentElement;
    }
  ...
  function bindScrollLockUserIntents() {
    if (isGrokScrollLockUserIntentsBound()) return;
    setGrokScrollLockUserIntentsBound(true);
  ...
  function installScrollGuards() {
    if (isGrokScrollGuardsInstalled()) return;
    setGrokScrollGuardsInstalled(true);
  ...
  function watchSendEvents(ui) {
    if (isGrokSendEventsBound()) return;
    setGrokSendEventsBound(true);
    ...
    document.addEventListener('click', (e) => {
      if (e.target && e.target.closest && e.target.closest('[data-testid="send-button"]')) {
        ...
        if (!currentUi) return;
        if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 检测到发送按钮点击，启动突发刷新');
    ...
    document.addEventListener('submit', (e) => {
      ...
      const currentUi = getUi();
      if (!currentUi) return;
      if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 检测到表单提交，启动突发刷新');
      armScrollLockGuard(2200);
      startBurstRefresh(currentUi);
    }, true);
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (isTextarea && e.key === 'Enter' && (e.metaKey || e.ctrlKey)) {
        const currentUi = getUi();
        if (!currentUi) return;
        if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 检测到快捷键发送，启动突发刷新');
        armScrollLockGuard(2200);
        startBurstRefresh(currentUi);
      }
    }, true);
    ...
    document.addEventListener('visibilitychange', () => {
      if (!document.hidden) {
        const currentUi = getUi();
        if (!currentUi) return;
        if (DEBUG || window.DEBUG_TEMP) console.log('Grok QuickNav: 页面重新可见，强制刷新');
