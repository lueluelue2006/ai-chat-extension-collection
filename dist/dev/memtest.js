(() => {
  ...
  const REGISTRY = globalThis.AISHORTCUTS_REGISTRY;
  if (!REGISTRY || !Array.isArray(REGISTRY.sites) || !REGISTRY.modules) {
    writeLog('[FATAL] AISHORTCUTS_REGISTRY not found. Did shared/registry.js load?');
    return;
  }
  ...
  function broadcastMemtestStatus(extra) {
    try {
      const payload = {
        type: 'AISHORTCUTS_MEMTEST_STATUS',
        at: Date.now(),
  ...
  async function getMenuCommands(tabId) {
    try {
      const resp = await withTimeout(tabsSendMessage(tabId, { type: 'AISHORTCUTS_GET_MENU' }), 5000, 'tabsSendMessage(GET_MENU)');
  ...
  async function runMenuCommandByName(tabId, name) {
    ...
    try {
      const resp = await withTimeout(
        tabsSendMessage(tabId, { type: 'AISHORTCUTS_RUN_MENU', id: cmd.id }),
        15_000,
  ...
  async function setSettings(settings) {
    ...
    const resp = await sendMessage({ type: 'AISHORTCUTS_SET_SETTINGS', settings, noInject: true });
    if (!resp || resp.ok !== true) throw new Error(resp?.error || 'Failed to save settings');
    return resp.settings;
  }

  async function patchSettings(patch) {
    const resp = await sendMessage({ type: 'AISHORTCUTS_PATCH_SETTINGS', patch, noInject: true });
          ...
          try {
            ...
            while (!state.stoppedAt && now() < endAt) {
              ...
              if (want > 0 && state.sent < want) {
                ...
                if (enabled.includes('cmdenter_send') && idx === 1) {
                  try {
                    const el = composer?.el;
                    if (el) {
                      usedHotkeySend = true;
  ...
  function applyUrlParams() {
    try {
      ...
      if (memguardClose === '1' || String(memguardClose || '').toLowerCase() === 'true') {
        ...
        void sendMessage({ type: 'AISHORTCUTS_MEMTEST_GUARD', reason: 'memguard_close' }).catch(() => void 0);
        closeSelfTabSoon(900);
        return;
      }
      ...
      if (memguardAbort === '1' || String(memguardAbort || '').toLowerCase() === 'true') {
        ...
        void sendMessage({ type: 'AISHORTCUTS_MEMTEST_ABORT', reason: 'memguard_abort' }).catch(() => void 0);
        closeSelfTabSoon(900);
        return;
      }
  ...
  try {
    chrome.runtime.onMessage.addListener((msg) => {
      try {
        ...
        if (type === 'AISHORTCUTS_MEMTEST_GUARD_EVENT') {
        ...
        if (type !== 'AISHORTCUTS_MEMTEST_ABORT') return;
