(() => {
  ...
  const EXACT_TEXT_MAP = Object.freeze({
    ...
    '搜索站点、脚本名称、快捷键或功能关键词。': 'Search sites, script names, hotkeys, or feature keywords.',
    '清空搜索': 'Clear search',
    '设置筛选': 'Settings filters',
    '全部': 'All',
    '快捷键': 'Hotkeys',
    '有细项': 'Detailed',
    '常用入口': 'Common entries',
    ...
    '保存后会自动生效到已打开页面；关闭功能后，已打开页面通常需要刷新才会完全停用。': 'Changes take effect on already-open pages automatically. After turning a feature off, already-open pages usually need a refresh to fully stop it.',
    ...
    '搜索结果会自动缩小左侧网站和中间脚本列表；选择脚本后在右侧编辑细项。': 'Search narrows the site and script lists automatically; choose a script to edit details on the right.',
    ...
    '默认不再内置任何探测 URL；保存下方列表后，每 1 小时请求一次每个 URL（目前仅支持 developers.openai.com/images/api/models/icons/...）。只有 HTTP 2xx 成功响应才会提醒你；403、404、5xx 和请求失败都视为不可用。要停止提醒，请删除全部 URL 并保存（空列表会停用检测/提醒；横幅不会被点击关闭）。': 'No probe URLs are built in by default anymore. After you save the list below, each URL is checked once per hour (currently only developers.openai.com/images/api/models/icons/...). Only HTTP 2xx success responses trigger alerts; 403, 404, 5xx, and request failures are treated as unavailable. To stop alerts, delete all URLs and save. An empty list disables checks and alerts; the banner itself cannot be dismissed by clicking.',
    ...
    '说明：仅在 Google 搜索页显示“问 GPT”按钮；点击后会新开 ChatGPT，并自动发起联网搜索提问。': 'Only shows the “Ask GPT” button on Google Search pages. Clicking it opens ChatGPT and automatically sends a web-search prompt.',
    ...
    '说明：兼容旧版 Genspark AI Chat 的 Sonnet 4.5 thinking 请求改写。该模块默认开启，但只会改写已知 Sonnet 4.5 模型，并在回复区内联显示“可展开思考块（默认仅最后 5 行）”。': 'Compatibility rewrite for legacy Genspark AI Chat Sonnet 4.5 thinking requests. This module is enabled by default, but only rewrites known Sonnet 4.5 models and shows collapsible thinking blocks inline in replies (last 5 lines by default).',
    ...
    '监控到资源可用时，在网页内显示大横幅（避免系统通知被屏蔽）': 'Show a large in-page banner when a monitored resource becomes available.',
    'ChatGPT 推理强度/模型 快捷切换': 'ChatGPT Reasoning / Model Hotkeys',
    '⌘O 推理强度 / ⌘J 模型切换': '⌘O reasoning effort / ⌘J model switch',
    '⌘O Medium/High / ⌘J High/Pro / 可选 High→max / 禁用 ⌘P / ⌘⇧P Light Pro 发送 / ⌘⌥P Heavy Pro 发送': '⌘O Medium/High / ⌘J High/Pro / optional High→max / block ⌘P / ⌘⇧P Light Pro send / ⌘⌥P Heavy Pro send',
    ...
    '选区多段 Quote / KaTeX 还原 LaTeX / 悬停提示 / 双击复制': 'Multi-quote selected text / restore KaTeX to LaTeX / hover hints / double-click copy.',
    '选区多段 Quote / KaTeX 还原 LaTeX / 悬停提示 / 双击复制（可配置）': 'Multi-quote selected text / restore KaTeX to LaTeX / hover hints / double-click copy (configurable).',
    ...
    '在 chatgpt.com：⌘O 在 Medium / High 间切换；⌘J 在 High / Pro 间切换。': 'On chatgpt.com, ⌘O switches between Medium / High, and ⌘J switches between High / Pro.',
    ...
    '在 Google 搜索框旁加“问 GPT”：跳到 ChatGPT 并自动发起联网搜索提问': 'Adds an “Ask GPT” button next to the Google search box, opens ChatGPT, and sends a web-search prompt automatically.',
    'Genspark 绘图默认设置': 'Genspark image defaults',
    '进入绘图页自动选择 GPT Image 2、4K、Medium，并关闭 Auto Prompt': 'On image pages, automatically select GPT Image 2, 4K, Medium, and turn off Auto Prompt.',
    ...
    'Genspark Sonnet 4.5 Thinking 兼容': 'Genspark Sonnet 4.5 Thinking compatibility',
    '仅在旧 Sonnet 4.5 请求上改写为 thinking，并显示可折叠思考块': 'Rewrite only legacy Sonnet 4.5 requests to thinking and show collapsible thinking blocks.',
    '回到顶部': 'Back to top',
    '回到底部': 'Back to bottom',
    ...
    '自动滚动锁定已开启': 'Auto-scroll lock is on',
    '自动滚动锁定已关闭': 'Auto-scroll lock is off',
    '自动滚动锁定关闭（点击开启）': 'Auto-scroll lock off (click to enable)',
    ...
    '清空搜索或切换筛选后继续配置脚本。': 'Clear the search or switch filters to keep configuring scripts.',
    '调整搜索词，切换筛选，或选择其他网站。': 'Adjust the search term, switch filters, or choose another site.',
    ...
    '提示：该模块会在页面主世界（MAIN world）监听 ⌘O/⌘J（可分别关闭），并默认拦截 ⌘P，避免误开浏览器打印；发送成功后右下角弹窗显示实际使用的 thinking_effort（以及 model）。关闭模块后已打开页面可能需要刷新才会完全停用。若你把键盘能力设为“无 Meta 键”，这里只会保留模块注入，⌘O/⌘J 默认停用。': 'This module listens for ⌘O / ⌘J in the page MAIN world (each can be disabled separately), and blocks ⌘P by default to prevent accidental browser print dialogs. It shows the actual thinking_effort and model in a bottom-right toast after a successful send. If you disable the module, already-open pages may need a refresh to fully stop it. If keyboard capability is set to “no Meta key”, the injection stays but ⌘O / ⌘J are disabled by default.',
    '提示：该模块会在页面主世界（MAIN world）监听 ⌘O/⌘J（可分别关闭），并默认拦截 ⌘P，避免误开浏览器打印；可选的 High→max 会在发送请求前把 High 对应的 thinking_effort 改为 max。关闭模块后已打开页面可能需要刷新才会完全停用。若你把键盘能力设为“无 Meta 键”，这里只会保留模块注入，⌘O/⌘J 默认停用。': 'This module listens for ⌘O / ⌘J in the page MAIN world (each can be disabled separately), and blocks ⌘P by default to prevent accidental browser print dialogs. Optional High→max rewrites the High request thinking_effort to max before sending. If you disable the module, already-open pages may need a refresh to fully stop it. If keyboard capability is set to “no Meta key”, the injection stays but ⌘O / ⌘J are disabled by default.',
    ...
    '增强 ChatGPT 的复制/引用：选区可追加多段 Markdown Quote，并优先还原 KaTeX 的原始 LaTeX。': 'Improve ChatGPT copy/quote behavior: append selected passages as stacked Markdown quotes and prefer original LaTeX from KaTeX.',
    ...
    '说明：该模块在页面主世界（MAIN world）通过事件驱动处理复制/引用：选中文本后显示轻量 Quote 按钮，点击会把本段追加为 Markdown 块引用，可重复追加多段；选区含 .katex 时优先还原 LaTeX。复制时仅在选区含 .katex 时改写剪贴板；点击原生 Quote 时仍只对该次引用做 LaTeX 补丁替换（不全局重载 Range/Selection）。交互：悬停公式 0.8s 显示 LaTeX 提示，双击公式复制 LaTeX 并弹出提示。关闭模块后已打开页面可能需要刷新才会完全停用。': 'This module handles copy/quote in the page MAIN world through events. Selecting text shows a lightweight Quote button; clicking it appends that passage as a Markdown blockquote, and repeated clicks stack multiple quotes. If the selection contains .katex, it prefers restored LaTeX. Copy only rewrites the clipboard when the selection contains .katex; native Quote still receives a one-shot LaTeX patch without global Range/Selection overrides. Interactions: hover over a formula for 0.8s to show the LaTeX tooltip, and double-click a formula to copy LaTeX with a toast. If you disable it, already-open pages may need a refresh to fully stop it.',
    '交互选项': 'Interaction options',
    '这些开关会实时写入配置；已打开的 ChatGPT 页面通常会在下一次选择、复制、悬停或点击时生效。': 'These switches are saved immediately; already-open ChatGPT pages usually apply them on the next selection, copy, hover, or click.',
    '选中文本后显示 Quote 按钮': 'Show the Quote button after selecting text',
    '关闭后不会弹出自定义 Quote；原生 Quote 不受这个开关影响。': 'When off, the custom Quote button will not appear; native Quote is not affected by this switch.',
    '显示自定义 Quote 时隐藏原生 Quote': 'Hide native Quote while showing the custom Quote',
    '关闭后保留 ChatGPT 原生 Quote/Ask ChatGPT 控件，适合对比或临时回退。': 'When off, ChatGPT’s native Quote / Ask ChatGPT controls remain visible for comparison or temporary fallback.',
    '原生 Quote 也尝试替换为 LaTeX': 'Also patch native Quote to LaTeX',
    '只在选区包含 KaTeX 公式时短时间补丁输入框；关闭后原生 Quote 完全按 ChatGPT 自己的逻辑处理。': 'Only patches the composer briefly when the selection contains KaTeX. When off, native Quote stays entirely under ChatGPT’s own behavior.',
    '复制含公式选区时写入 LaTeX': 'Write LaTeX when copying selections with formulas',
    '关闭后不拦截 copy 事件，浏览器和 ChatGPT 原生复制结果保持原样。': 'When off, copy events are not intercepted, so browser and ChatGPT native copy output stays unchanged.',
    '悬停公式显示 LaTeX 提示': 'Show LaTeX tooltip on formula hover',
    '关闭后不会显示公式悬停气泡，也会清掉已经显示的提示。': 'When off, formula hover bubbles are not shown and any existing tooltip is cleared.',
    '双击公式复制 LaTeX': 'Double-click formula to copy LaTeX',
    '关闭后双击公式不再写剪贴板，避免误触覆盖剪贴板。': 'When off, double-clicking formulas no longer writes to the clipboard, avoiding accidental clipboard overwrite.',
    '说明：该模块在页面主世界（MAIN world）通过事件驱动处理复制/引用，不全局重载 Range/Selection。默认行为保持开启：选中文本后显示轻量 Quote 按钮，可重复追加多段；选区含 .katex 时优先还原 LaTeX；复制含公式选区时改写剪贴板；悬停公式 0.8s 显示 LaTeX 提示；双击公式复制 LaTeX。关闭模块注入后，已打开页面可能需要刷新才会完全停用。': 'This module handles copy/quote in the page MAIN world through events and does not globally override Range/Selection. Default behavior remains on: selected text shows a lightweight Quote button that can append multiple passages; selections containing .katex prefer restored LaTeX; copying formula selections rewrites the clipboard; hovering formulas for 0.8s shows a LaTeX tooltip; double-clicking formulas copies LaTeX. After disabling module injection, already-open pages may need a refresh to fully stop it.',
    ...
    '仅在绘图页面生效：进入页面自动选择 GPT Image 2、4K、Medium，并关闭 Auto Prompt。': 'Only active on image-generation pages: automatically select GPT Image 2, 4K, Medium, and turn off Auto Prompt.',
    '说明：该模块在新版 AI Image 入口和旧绘图入口生效；会尽量通过按钮文本、aria-label、弹窗选项等启发式方式选择 GPT Image 2、4K、Medium，并关闭 Auto Prompt。若关闭模块，已打开页面可能需要刷新才会完全停用。': 'This module is active on the new AI Image entry and the legacy image entry. It uses button text, aria-labels, dialog options, and other heuristics to select GPT Image 2, 4K, Medium, and turn off Auto Prompt. If you disable it, already-open pages may need a refresh to fully stop it.',
    ...
    '说明：当前只支持纯文本队列；Tab 会拦截原生焦点切换并把当前草稿排进队列，Shift+Tab 仅负责阻止浏览器切焦点，不附带额外语义，⌥↑ / Alt+↑ 取回最近一条已排队草稿。排队预览会显示最多 10 条草稿，并提供取回最新、清空队列、单条强发和单条删除。自动发送下一条时，主判定来自 conversation stream 的 [DONE]，不会只看发送按钮是否高亮；Ctrl+C 清空走浏览器编辑命令，尽量保留 Cmd+Z 撤销链；QuickNav 橙色标记会在点击对应消息后清掉。': 'Only plain-text queue items are currently supported. Tab intercepts the native focus jump and queues the current draft. Shift+Tab only prevents the browser focus change and adds no extra behavior. ⌥↑ / Alt+↑ restores the latest queued draft. The queue preview shows up to 10 drafts and provides restore latest, clear queue, force-send item, and remove item actions. Auto-sending the next item is mainly gated by conversation stream [DONE] rather than only the highlighted send button. Ctrl+C clears the composer through the browser editing command to preserve the Cmd+Z undo chain as much as possible. The orange QuickNav mark is cleared after you click the corresponding message.',
    '正在加载设置': 'Loading settings',
    ...
    'High 发送时改写 thinking_effort=max': 'Rewrite thinking_effort=max when sending High',
    '禁用 ⌘P（阻止浏览器打印）': 'Disable ⌘P (prevent browser print)',
  ...
  const REGEX_REPLACEMENTS = [
    ...
    [/^(.+) · (\d+)\/(\d+) 个脚本匹配$/, '$1 · $2/$3 scripts matched'],
    [/^(.+)：(\d+)\/(\d+) 个网站，(\d+)\/(\d+) 个脚本。$/, '$1: $2/$3 sites, $4/$5 scripts.'],
    [/^(.+) 没有符合当前搜索和筛选条件的脚本。$/, '$1 has no scripts matching the current search and filters.'],
    [/^检测到 (\d+) 条资源可用（每次检测都会提醒）：(.+)$/, '$1 monitored resources are available: $2'],
