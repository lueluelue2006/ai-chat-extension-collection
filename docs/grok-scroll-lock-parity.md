...
- 回归测试
  ...
  - `dev/scroll-tests/grok-scroll-lock-smoke.js`（浏览器烟测脚本）
...
- `npm run check`
  - 覆盖 registry/injection 一致性、manifest 对齐、脚本语法、dev self-tests
  - 已包含 Grok 新增的两条静态回归测试

### 4.2 浏览器烟测（Grok 页面）

在 `https://grok.com/c/...` 打开 DevTools Console 后执行：

- `dev/scroll-tests/grok-scroll-lock-smoke.js`

脚本会输出结构化结果，包含：

1. lock 按钮状态与 dataset 是否一致
2. `QUICKNAV_SCROLLLOCK_STATE` 是否能按 `channel/v/nonce` 捕获
3. 锁开启时程序下滚是否被拦截
4. （best-effort）切会话后锁状态是否保持
