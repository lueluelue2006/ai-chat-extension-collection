...
- 自动化验证链
  - 已补齐 Kimi scroll-lock 协议与注入路由的静态回归覆盖。

## 4. 验证口径（必须项）

1. 终端校验：`npm run check`、`npm test`、`npm run typecheck`、`npm run build`。
...
- 另有自动化配置覆盖，已验证注入匹配同时包含 `https://kimi.com/*` 与 `https://www.kimi.com/*`。
