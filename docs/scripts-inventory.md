...
- `hide_disclaimer`: 隐藏免责声明/提示条 — 自动隐藏“AI 可能会犯错/数据使用”等提示条
  - 作者: lueluelue2006
  - 许可证: 未标注（内部脚本）
  - 注入: document_start / ISOLATED: `content/common-hide-disclaimer/main.js`
...
- `chatgpt_hide_feedback_buttons`: ChatGPT 隐藏点赞/点踩 — 隐藏回复下方反馈按钮（👍/👎）
  - 作者: lueluelue2006
  - 许可证: 未标注（内部脚本）
  - 注入: document_start / ISOLATED: `content/chatgpt-hide-feedback-buttons/main.js`
- `chatgpt_tex_copy_quote`: ChatGPT TeX Copy & Quote — 复制/引用含 KaTeX 的选区时优先还原 LaTeX，并支持悬停提示/双击复制
  ...
  - 注入: document_start / MAIN: `content/chatgpt-tex-copy-quote/main.js`
- `chatgpt_export_conversation`: ChatGPT 对话导出（新版 UI） — 一键导出当前对话为 Markdown / HTML（在扩展菜单里执行）
  - 作者: lueluelue2006
  - 许可证: 未标注（内部脚本）
  - 注入: document_end / ISOLATED: `content/menu-bridge.js`, `content/chatgpt-export-conversation/main.js`
- `chatgpt_image_message_edit`: ChatGPT 消息分叉编辑（可加图） — 给用户消息增加“分叉编辑”按钮：在输入框里编辑并可补图/文件；与原生编辑共存
  - 作者: lueluelue2006
  - 许可证: 未标注（内部脚本）
  - 注入: document_start / MAIN: `content/chatgpt-fetch-hub/main.js`, `content/chatgpt-image-message-edit/main.js`
...
- `chatgpt_cmdenter_send`: ⌘Enter 发送（Enter 换行） — Enter/Shift+Enter 换行（强制）
  ...
  - 许可证: 未标注（内部脚本）
  - 注入: document_start / ISOLATED: `content/chatgpt-cmdenter-send/main.js`
- `gemini_math_fix`: Gemini Enterprise 数学修复 — KaTeX / inline math 修复
  - 作者: lueluelue2006
  - 许可证: 未标注（内部脚本）
  - 注入: document_start / MAIN: `content/gemini-enterprise-math-fix/main.js`
- `gemini_auto_3_pro`: Gemini Enterprise 自动切换 3 Pro — 自动将模型切换为 Gemini 3 Pro（可用时）；并隐藏 Gemini Enterprise 的 disclaimer
  - 作者: lueluelue2006
  - 许可证: 未标注（内部脚本）
  - 注入: document_end / ISOLATED: `content/gemini-enterprise-auto-gemini-3-pro/main.js`
...
- `genspark_credit_balance`: Genspark 积分余量 — 悬停小蓝点显示积分信息（可刷新/折叠/拖动）
  ...
  - 许可证: 未标注（内部脚本）
  - 注入: document_end / ISOLATED: `content/genspark-credit-balance/main.js`
...
- `grok_fast_unlock`: Grok 4 Fast 菜单项 — 在模型菜单增加 “Grok 4 Fast”，并在发送时选用该模型
  ...
  - 许可证: 未标注（内部脚本）
  - 注入: document_start / MAIN: `content/grok-fast-unlock/main.js`
- `grok_rate_limit_display`: Grok 剩余次数显示 — 在输入框附近显示 rate limit（剩余次数/等待时间）
  ...
  - 许可证: 未标注（内部脚本）
  - 注入: document_end / MAIN: `content/grok-rate-limit-display/main.js`
