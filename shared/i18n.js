(() => {
  ...
  const EXACT_TEXT_MAP = Object.freeze({
    ...
    '说明：兼容旧版 Genspark AI Chat 的 Sonnet 4.5 thinking 请求改写。当前 Genspark 默认模型已切到 Opus 4.6，因此该模块默认关闭；手动开启后只会改写已知 Sonnet 4.5 模型，并在回复区内联显示“可展开思考块（默认仅最后 5 行）”。': 'Compatibility rewrite for legacy Genspark AI Chat Sonnet 4.5 thinking requests. Current Genspark defaults to Opus 4.6, so this module is off by default; when manually enabled, it only rewrites known Sonnet 4.5 models and shows collapsible thinking blocks inline in replies (last 5 lines by default).',
    ...
    '仅在旧 Sonnet 4.5 请求上改写为 thinking，并显示可折叠思考块（默认关闭）': 'Rewrite only legacy Sonnet 4.5 requests to thinking and show collapsible thinking blocks (off by default).',
    '回到顶部': 'Back to top',
    '回到底部': 'Back to bottom',
